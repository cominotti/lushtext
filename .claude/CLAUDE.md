...
- **Sidebar position constraint**: Three-layer defense against GTK "Trying to measure GtkBox for width of X, but needs at least Y" warnings (caused by GTK4's `measure()` running BEFORE `size_allocate()`, using stale paned positions): (1) **Pre-clamp in `constructed()`** — after restoring sidebar position from GSettings, immediately validates it against the restored window width and `content_stack.measure(Horizontal, -1)` minimum. The original unclamped position is preserved in `saved_sidebar_pos` for show-animation targets at wider widths. (2) **Explicit `content_box.width_request`** — set to `stack_min` at construction, making the paned's minimum constraint visible in the widget tree. (3) **Runtime `clamp_sidebar_position()`** — enforces `position <= min(width / 3, width - stack_min - handle_overhead)`. `handle_overhead` is computed at construction from `paned_min - sidebar_min - stack_min` (replaces the former hardcoded 16px). Called from `size_allocate()` (BEFORE `parent_size_allocate`, the primary defense) and `notify::position` (catches user drag). Guards: `window_width <= 0` for pre-realization safety, `sidebar_visible.get() == false` to skip when hidden, `.max(0)` to prevent negatives. A `width-request=640` on the window template prevents geometrically impossible layouts.
