...
- **Async save**: `save_file_async` moves `std::fs::write` to a background thread via `spawn_blocking_then`. Sets `buffer.set_modified(false)` optimistically before the write for instant visual feedback (tab dot disappears immediately); on write failure, rolls back via `set_modified(true)`. Callers pass a callback for success/error handling. This prevents UI freezes on slow filesystems (NFS, USB).
