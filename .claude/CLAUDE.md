...
- **Sidebar context menu**: Right-click on a file or directory shows a `GtkPopoverMenu` (backed by `gio::Menu`) with Rename and Delete actions. A single `GtkGestureClick(button=3)` on the `ListView` uses `Widget::pick()` + ancestor traversal to find the `TreeExpander` → `TreeListRow` → `FileTreeItem` at click coordinates. Context state (path, is_dir, expander ref) is stored on the sidebar imp for action handlers. Actions are in a `sidebar` action group installed via `insert_action_group`.
