...
- [x] 3.1 Replace `set_aside::list` with the bounded single-traversal `SetAsideListing` (design D6), and update `draft_service::list_set_aside_drafts`. Verify: 1.1 passes, and existing set-aside tests pass.
- [x] 3.2 Add `set_aside::delete_confirmed` (revalidate fingerprint, `ensure_inside`, one directory sync per batch, partial-failure count). Update the `set_aside.rs` module doc's ownership rule. Verify with unit tests, including a mismatched-fingerprint case that deletes nothing.
...
- [x] 4.3 Run the state-extreme checks: empty, 1 body, many bodies, truncated, over bound, and constrained dialog geometry. Commands stay reachable, and only the details region scrolls.
...
- [x] 6.1 Update `docs/accessibility.md` and `docs/accessibility-matrix.md` (row `A11Y-PREFERENCES-DATA-SET-ASIDE` for the summary row and the Delete All confirmation, plus a new row for the over-bound notice and the review action), and `docs/accessibility-orca-checklist.md` if the manual expectations change.
