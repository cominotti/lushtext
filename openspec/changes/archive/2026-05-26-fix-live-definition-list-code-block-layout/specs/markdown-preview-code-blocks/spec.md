...
hidden when the code block was rendered. Horizontal scrolling MUST appear only
when a code line is wider than the effective available preview text column after
padding and nested context margins are accounted for.
...
#### Scenario: Suppress false horizontal overflow after opening the side-by-side preview pane
- **WHEN** the user opens the side-by-side Markdown preview pane for a Markdown document containing a definition-list definition with a nested code block whose longest line fits the pane's visible definition-body column
- **THEN** the nested code block settles to the definition body's effective available code-block column after the preview pane becomes visible
- **AND** the code block does not allocate as a tiny natural-width clipped box
- **AND** the code text is legible without horizontal scrolling
...
- **WHEN** the Markdown preview width, preview pane position, preview visibility, or readable-column margins change after a code block has been rendered
