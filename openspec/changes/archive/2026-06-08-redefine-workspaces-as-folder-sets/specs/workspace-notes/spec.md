...
- **AND** newly written domain-facing code does not name that note as a workspace-root note
...
#### Scenario: Naming tests cover old workspace-note concepts
- **WHEN** the implementation is complete
- **THEN** tests, fixtures, UI resources, and developer documentation no longer present folder-note workflows as workspace-root notes
...
**Reason**: A workspace no longer owns one singular root. The note attached to a configured folder is now a folder note.
**Migration**: Existing valid workspace-root note data migrates or loads compatibly as folder-note data for the same canonical folder identity.

### Requirement: Workspace notes support edit and rendered markdown reading modes
**Reason**: The editing and rendering behavior remains, but the domain object is now a folder note.
**Migration**: Replace this requirement with `Folder notes support edit and rendered markdown reading modes`.

### Requirement: Workspace-note persistence follows workspace-root identity
**Reason**: Folder-note persistence follows canonical folder identity, independent of workspace label, workspace slot, or folder order.
**Migration**: Preserve existing canonical path identities while renaming model/service/UI concepts to folder-note terminology.

### Requirement: Workspace-note sidecars use the public v1 JSON envelope
**Reason**: The old requirement names the domain concept as a workspace note for a workspace root.
**Migration**: Keep only an explicit compatibility path for existing sidecars if needed, and write/read the domain through folder-note terminology.

### Requirement: Workspace-note browser entries use the native Adwaita sidebar rail
**Reason**: Browser entries now represent folder notes, not workspace-root notes.
**Migration**: Replace with folder-note sidebar rail requirements and update labels, tests, fixtures, and preview code.

### Requirement: Workspace-note editor mode switching is layout-stable
**Reason**: The layout-stability behavior remains, but it applies to folder-note editors.
**Migration**: Replace with the folder-note editor mode-switching requirement.

### Requirement: Workspace-note sidecar corruption is isolated and diagnostic
**Reason**: Recovery behavior remains, but the persisted user concept is a folder note.
**Migration**: Route legacy sidecar diagnostics through folder-note recovery wording except in low-level compatibility logs where the old sidecar kind is unavoidable.

### Requirement: Workspace-note root migrations are retryable
**Reason**: The migration target is no longer a workspace root.
**Migration**: Replace with retryable folder-note migration coverage for folder renames and legacy workspace-note sidecar compatibility.

### Requirement: Workspace-note reconciliation preserves root-note content
**Reason**: Reconciliation must preserve folder-note content attached to canonical folder identity, not root-note content attached to a singular workspace root.
**Migration**: Replace with folder-note reconciliation requirements.

### Requirement: Workspace-note reliability has layered automated coverage
**Reason**: The coverage target is now folder-note behavior plus terminology cleanup.
**Migration**: Replace with folder-note reliability coverage that includes a naming audit for old workspace-note and workspace-root concepts.
