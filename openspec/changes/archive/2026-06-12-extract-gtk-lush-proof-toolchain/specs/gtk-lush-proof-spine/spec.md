...
- **WHEN** a proof client asks a provider for an unsupported readiness predicate
...
- **WHEN** Automation1 introspection is captured before and after the spine
  migration
- **THEN** the object path, interface name, method names, property names, signal
  names, and existing type signatures are unchanged except for explicitly
