...
- **AND** the menu shows the current-document entry points `Add Bookmark` or `Remove Bookmark`, `Add Range Note…`, and `Open Document Note…`
- **AND** the menu shows the workspace entry points `Open Workspace Note…` and `Export Range Notes…`
- **AND** the menu does not show `Browse Bookmarks…`, `Edit Bookmark Label…`, or `Edit Range Note…`
...
- **AND** the bookmark toggle, `Add Range Note…`, and `Open Document Note…` are insensitive
- **AND** `Open Workspace Note…`, `Browse Notes…`, and `Export Range Notes…` remain actionable
...
- **AND** `Browse Notes…` and `Export Range Notes…` remain actionable
...
- **WHEN** the editor context identifies an existing bookmark or range note at the cursor
- **THEN** the editor context menu offers the corresponding edit action
- **AND** activating that action routes through the existing bookmark-label or range-note editor workflow
