...
The system SHALL scope workspace-aware search and palette indexing to the current shared workspace scope. When a concrete workspace is selected, those features MUST use only that workspace's root directory. When `All workspaces` is selected, they MUST aggregate the roots of every workspace.
...
- **THEN** the feature searches across the roots of every restored workspace
