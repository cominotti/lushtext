...
- [x] 3.1 Push the branch, open a draft PR, and collect at least two CI runs of every shard
- [x] 3.2 Verify from CI logs that the per-shard selected counts sum to the binary's total
- [x] 3.3 Record the per-shard maxima and run ids in the table; `make check-widget-shards` passes with every shard ≤ 20 min

## 4. Documentation and verification

- [x] 4.1 Update `.agents/rules/build.md` (CI job list, widget budget paragraph, rules index text if needed), `AGENTS.md`/`.claude/CLAUDE.md` testing and CI text, and the `gtk-testing` skill
- [x] 4.2 `make check` and `make test` pass locally
- [x] 4.3 `openspec validate shard-widget-test-ci-job` passes; record deviations in design.md
