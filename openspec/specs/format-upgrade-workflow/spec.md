...
The system SHALL add a `Preferences > Data` page that lets the user inspect current app-owned metadata format status, manually rescan, retry applicable upgrades, and see concise recovery/backup state. The page SHALL render correctly for empty/current state, representative upgradeable state, many or awkward items, failed conversion state, future-version state, and constrained dialog geometry.
...
- **AND** no destructive or irrelevant action is presented as required
