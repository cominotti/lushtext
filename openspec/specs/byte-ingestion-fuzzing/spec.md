# byte-ingestion-fuzzing Specification

## Purpose
Define LushText's byte-ingestion fuzzing lane so arbitrary bounded bytes can
exercise deterministic editor decoding and Markdown preprocessing surfaces
without pulling fuzz-only dependencies, sanitizer settings, GTK widgets, or
desktop-session requirements into default validation.
