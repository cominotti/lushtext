...
## Purpose
Define the shared current workspace scope that the sidebar, search, palette indexing, and workspace-aware note workflows all honor consistently.
## Requirements
...
The system SHALL scope workspace-aware note and bookmark workflows to the current shared workspace scope. A concrete workspace scope MUST limit those flows to one workspace root. The aggregate scope MUST include all restored workspaces.

#### Scenario: Note browsers stay inside the selected workspace
...
- **THEN** the browser includes data from every restored workspace
