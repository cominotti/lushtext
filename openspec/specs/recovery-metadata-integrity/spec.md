...
The system SHALL distinguish valid, missing, malformed, unreadable, non-file, oversized, and partially repairable app-owned recovery metadata. Recovery metadata includes session state, draft manifests, note and bookmark sidecars, local-history indexes, Replace All undo journals, and migration ledgers. A caller that loads recovery metadata MUST receive structured diagnostics instead of silently treating every failure as empty default state.
