...
## Purpose
Validate a file-backed draft against its backing file's freshness before restoring it, and preserve a stale draft recoverably (set-aside copy plus local history where policy accepts it) instead of destroying it when the file changed externally.
## Requirements
