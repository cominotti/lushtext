...
- Keep file load/save flows in `load_save.rs`, external monitor behavior in `monitor.rs`, and in-tab search-bar behavior in `search.rs`.
...
- Keep `last_known_mtime`, eviction state, draft flags, and EditorConfig override state explicit and well-named; these are coupled safety/restore signals, not incidental fields.
