...
- Keep file load/save flows in `load_save.rs`, minimap behavior in `minimap.rs`, bookmark projection in `bookmarks.rs`, annotation projection in `annotations.rs`, external monitor behavior in `monitor.rs`, and in-tab search-bar behavior in `search.rs`.
