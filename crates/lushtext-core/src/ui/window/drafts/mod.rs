...
//! | `drafts.manifest`, `manifest_authority`, `preloaded` | **owned here.** The session workflow's startup read produces them and hands them over through `adopt_startup_draft_records`, one named operation rather than three field writes from another workflow's file. Every commit, authority change, and cleanup removal is mirrored into the other windows' copies (`adopt_draft_manifest`) |
