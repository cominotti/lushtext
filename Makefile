...
#   make gtk-axioms - Run every gtk-lush-axioms probe and sample --check under private headless Mutter
#   make gtk-axiom-sample AXIOM=<id> [CHECK=1] - Open one axiom sample (CHECK=1: headless verdict)
#   make gtk-axioms-runtimes - LOCAL ONLY: rerun sample checks inside installed GNOME SDKs (50, nightly)
#   make check-gtk-axioms - Keep the GTK axiom ledger, catalogue, probes, and samples in agreement
...
GTK_LUSH_PACKAGES := -p gtk-lush-axioms -p gtk-lush-signals -p gtk-lush-settle -p gtk-lush-tasks -p gtk-lush-viewport -p gtk-lush-widgets -p gtk-lush-proof-harness -p gtk-lush-proof-spine
GTK_LUSH_CRATES := crates/gtk-lush/axioms crates/gtk-lush/signals crates/gtk-lush/settle crates/gtk-lush/tasks crates/gtk-lush/viewport crates/gtk-lush/widgets crates/gtk-lush/proof-harness crates/gtk-lush/proof-spine
...
# Kani formal verification lane. The version is pinned here and only here:
# `.github/workflows/kani.yml` reads it through `scripts/kani-shards.py
# github-outputs`. Kani pins its own nightly, so the stable workspace toolchain is untouched. Proof
# builds use their own target directory so they never churn `target/`.
KANI_VERSION ?= 0.68.0
...
# Every `#[kani::proof]` harness in the crates that own checked code. Fails when
# the installed Kani is not the pinned version or when any harness fails;
...
# The kill-point scenarios need a second debug binary with the
# crash-kill-points feature, in its own target directory so the ordinary debug
# binary (and every release, Meson, Flatpak, and Snap build) never carries it.
...
# --bins, not --all-targets: the lab's axiom_probes test target needs a
# private headless compositor, which `make gtk-axioms` provides.
gtk-lush-adoption-lab:
	@echo "Testing GTK Lush adoption lab..."
	cargo test $(GTK_LUSH_ADOPTION_LAB_PACKAGE) --bins

# The gtk-lush-axioms probe binary (one isolated test per probed axiom) and
# every sample's --check, all under a private `mutter --headless` session;
# never the live desktop.
gtk-axioms:
	@echo "Running GTK axiom probes and sample checks headless..."
	./scripts/run-gtk-axioms.sh all

# One sample: interactive on the current desktop session by default (for a
# person to watch), or its headless verdict with CHECK=1.
gtk-axiom-sample:
	@test -n "$(AXIOM)" || { echo "usage: make gtk-axiom-sample AXIOM=<id> [CHECK=1]" >&2; exit 2; }
	./scripts/run-gtk-axioms.sh sample "$(AXIOM)" $(if $(filter 1,$(CHECK)),--check,)

# LOCAL ONLY: rerun every sample --check against the toolkit of each installed
# GNOME SDK (default 50 and nightly), before that toolkit becomes the floor.
# Never a prerequisite of check, test, or any CI job; installs nothing.
gtk-axioms-runtimes:
	./scripts/run-gtk-axioms.sh runtimes $(GTK_AXIOMS_RUNTIMES)
...
help:
	...
	@echo "  gtk-axioms       Run every GTK axiom probe and sample --check headless"
	@echo "  gtk-axiom-sample Open one axiom sample: AXIOM=<id> [CHECK=1]"
	@echo "  gtk-axioms-runtimes LOCAL ONLY: sample checks inside installed GNOME SDKs"
	@echo "  check-gtk-axioms Keep the axiom ledger, catalogue, probes, and samples in agreement"
