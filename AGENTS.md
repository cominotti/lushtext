...
- `widget-wiring.md` — GTK widget composition, signal wiring, and live paned-validation rules
...
- **Sidebar toggle**: `GtkToggleButton` in the status bar (icon: `sidebar-show-symbolic`) with `action-name=win.toggle-sidebar` (stateful boolean action via `SimpleAction::new_stateful` + `connect_change_state`). F9 keyboard shortcut. The paned start-child is a `GtkRevealer` wrapper around `LushtextSidebar`, and `animate_sidebar(bool)` drives an `AdwTimedAnimation` on the paned position (250ms, `EaseOutCubic`) while the wrapper clips the real sidebar offstage. Hidden state is truly zero-width (`SIDEBAR_COLLAPSED_POSITION = 0`): startup-hidden restore sets the paned to 0 and hides the wrapper, and hide `connect_done` drops `reveal-child` and `visible` together after the paned animation finishes so GTK stops reserving handle width while the sidebar is offstage without shrinking the layout budget too early. Show targets come from `saved_sidebar_pos`, but the animated positions themselves are written directly once the show/hide endpoints are pre-clamped. The layout budget is refreshed on map, before sidebar animations, and after restored workspace mutations; once allocated, `GtkPaned::max_position()` is treated as the authoritative runtime cap. Rapid toggle is handled by pausing the in-flight animation and starting fresh from `paned.position()`. `saved_sidebar_pos: Cell<i32>` stores the preferred visible width (only overwritten when no animation is running, to avoid saving intermediate values). Visibility persisted via GSettings `sidebar-visible` key (default: `true`). `size_allocate` keeps geometry clamps live during the animation, but a dedicated `sidebar_animation_active` flag suppresses both debounced `sidebar-position` writes and the drag-oriented `notify::position` loop until the animation settles, so toggle animations do not reprocess every frame as both an animation tick and a synthetic drag. A cached `sidebar_visible: Cell<bool>` on the window imp struct avoids GObject property lookups in the `clamp_sidebar_position` hot path (~60Hz during resize). The clamp function early-returns only once the sidebar is fully hidden from layout, so hide-time transitions still get budget protection.
...
- **Sidebar position constraint**: Seven-layer defense against GTK "Trying to measure GtkBox for width of X, but needs at least Y" warnings (caused by GTK4's `measure()` running BEFORE `size_allocate()`, using stale paned positions): (1) **Pre-clamp in `constructed()`** — after restoring sidebar position from GSettings, immediately validates it against the restored window width and `content_box.measure(Horizontal, -1)` minimum. The original unclamped position is preserved in `saved_sidebar_pos` for wider windows. (2) **Explicit `content_box.width_request`** — set to `content_min` and refreshed when the realized end-child minimum changes, making the paned's minimum constraint visible in the widget tree. (3) **Hidden-state restore** — when the sidebar starts hidden, the live paned position is reset to the zero-width collapsed endpoint and the revealer wrapper is hidden so the first show animation starts from a valid hidden state with no reserved handle width. (4) **Wrapper-based zero-width animation** — the raw sidebar is never exposed directly to illegal intermediate paned widths; the `GtkRevealer` wrapper absorbs the fully hidden/offstage state instead. (5) **Animation-write clamping** — show targets and animation ticks are clamped against the current layout budget before `GtkPaned::set_position()` runs, using `main_paned.max_position()` once the paned is allocated and measured fallbacks before allocation. (6) **Hide-time layout participation** — runtime budget refreshes and clamps stay active until the revealer wrapper's own `visible` property turns false; the cached logical sidebar-visible flag flips at hide start and is too early to decide the pane is gone from layout. (7) **Runtime `clamp_sidebar_position()`** — enforces `position <= min(width / 3, max_position)` as the backstop for drags and live resize. `handle_overhead` is still refreshed from the live layout budget (`paned_min - sidebar_min - content_min`) on map, during fallback clamp calculations, and after workspace restore changes so stale construction-time measurements cannot leak a 1px invalid width. Called from `size_allocate()` (BEFORE `parent_size_allocate`) and `notify::position` (catches user drag). Guards: once the sidebar is fully hidden (logical flag false and wrapper hidden), the clamp exits early; otherwise it stays live during show/hide transitions. A `width-request=640` on the window template prevents geometrically impossible layouts.
...
- **Search progress reporting**: `SearchEvent::Progress(usize)` is emitted every 100 files by the search service via an `Arc<AtomicUsize>` file counter shared across `WalkParallel` threads. Best-effort via `try_send` to avoid blocking match delivery. The status bar has `set_progress_message(text)` and `clear_progress_message()` for non-auto-dismissing messages — `progress_active: Cell<bool>` ensures normal `push_message` always overrides progress. In `window/search.rs`, progress display uses a 500ms delay (feedback proportional to duration): `Rc<Cell<bool>>` flag set by `timeout_add_local_once(500ms)` — fast searches never show progress. Denominator "Y" in "Searching X / Y files..." comes from `command_palette.file_index_len()`.
