...
- **Status bar**: per-window bottom bar below the split-view shell, always visible. Four sections: workspace sidebar toggle button (far left), feedback message area (left, hexpand), encoding label (right), file size label (right). The window orchestrates all updates via `refresh_status_bar()`, which also refreshes the properties panel's document rows.
- **Dual split-view shell**: The outer window shell uses nested `AdwOverlaySplitView`s. `workspace_split_view` hosts `LushtextSidebar` on the left, and `properties_split_view` wraps the central editor/search host with `LushtextPropertiesPanel` on the right. The status-bar toggle stays bound to `win.toggle-sidebar` / `workspace_split_view:show-sidebar`; the header bar adds `win.toggle-properties` for the right pane. Breakpoints collapse the properties pane before the workspace pane, and both panes keep the same boolean toggle model in side-by-side and overlay modes.
