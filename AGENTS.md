...
│   ├── draft_service.rs + draft_service/ # Draft persistence plus typed, bounded orphan-cleanup evidence
...
- **Tab content transparency**: `Preferences > Editor > Appearance` includes an always-visible `Background Opacity` control patterned after modern Fedora terminal settings. The selected opacity applies to the main editor document surface and Markdown preview background, while lower values make those backgrounds more transparent. The header bar, tab bar chrome, side panels, status/search chrome, and minimap remain explicitly opaque. The workspace sidebar and document-properties panel share GNOME-like side-rail chrome on `@headerbar_bg_color`; unrelated opaque shell surfaces such as empty states, search bars, and preview placeholders stay on `@window_bg_color`.
...
- **Draft persistence**: Unsaved buffer content is periodically written to `$XDG_DATA_HOME/lushtext/drafts/` as plain UTF-8 text files. A JSON manifest (`manifest.json`) maps draft IDs to original paths and metadata (mtime, saved_at). Path-backed IDs use the explicit stable path hash shared with sidecar identities; untitled tabs use `"untitled-{counter}"`. Autosave and close safety use a backpressured pipeline that snapshots, writes, and releases one complete body before admitting the next; only compact entries survive to one final manifest commit. `draft_dirty` clears only after same-ID/same-generation body plus manifest acceptance, and every failure stays retryable. Automatic capture and bounded reads share a 64 MiB per-draft UTF-8 limit, while startup separately caps eager bodies at 64 MiB aggregate and retains compact lazy markers until slow session file loads can enter the serialized lazy-read queue. Startup orphan cleanup is a deferred two-phase workflow: bounded inspection uses recovery-aware status, execution reloads the latest trusted manifest, and the GTK completion merges only durably committed exact entry fingerprints. Missing-body entries, duplicate IDs, scan/status/delete/write failures, and post-inspection races preserve evidence. Orphan-body deletion acquires the stable target guard and validates the inspected inode immediately before removal so an atomic autosave replacement cannot be deleted during the body-write/manifest-publication gap. One delayed bounded follow-up may continue a large manifest page or retry preserved evidence without forming a tight loop.
