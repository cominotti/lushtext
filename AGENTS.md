...
- **Filesystem boundary**: Production code must use `services::filesystem` for reads, metadata, canonical identity, traversal, mutation, sidecar handling, and durable writes. Raw filesystem calls belong only in the private filesystem backend or fixture helpers; the private durable-write state machine delegates platform operations through that backend. Tests and benches should use `services::filesystem::fixture` so examples stay readable without teaching bypasses.
