...
- **Multi-workspace sidebar**: The sidebar is a two-level orchestrator. `LushtextSidebar` keeps a fixed "New Workspace" affordance at the top, a fixed width-preset footer at the bottom, and a scrollable list of `LushtextWorkspaceSection` widgets between them. Each section has its own `GtkListView` + `GtkTreeListModel` + `GtkTreeExpander` file tree. The top affordance and bottom footer stay visible while the workspace list scrolls, and the sidebar no longer exposes a horizontal scrollbar when long workspace or file names exceed the visible width. The footer hosts centered `Small`, `Comfy`, and `Large` buttons, with tooltips explaining the `20% / 30% / 40%` total-window targets that drive the left split-view preset. Clicking the top add-folder button creates a new workspace (auto-named from folder). Each section has its own add-folder button for adding more roots. Right-click on workspace header shows Rename/Unlist context menu.
- **Inner ScrolledWindow pattern**: Each `WorkspaceSection` wraps its `GtkListView` in a `GtkScrolledWindow(propagate-natural-height=true, propagate-natural-width=true, vscrollbar-policy=never, hscrollbar-policy=never)`. This provides the vadjustment that ListView requires while letting the row content size naturally even though the outer sidebar scroller now handles vertical scrolling only. Without this, GtkListView may crash or emit warnings.
