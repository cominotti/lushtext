...
- **Multi-workspace sidebar**: The sidebar is a two-level orchestrator. `LushtextSidebar` keeps a fixed "New Workspace" affordance at the top and a scrollable list of `LushtextWorkspaceSection` widgets below it. Each section has its own `GtkListView` + `GtkTreeListModel` + `GtkTreeExpander` file tree. The top affordance stays visible while the workspace list scrolls, and the outer sidebar scroller provides horizontal scrolling when long workspace or file names exceed the visible width. Clicking the top add-folder button creates a new workspace (auto-named from folder). Each section has its own add-folder button for adding more roots. Right-click on workspace header shows Rename/Unlist context menu.
- **Inner ScrolledWindow pattern**: Each `WorkspaceSection` wraps its `GtkListView` in a `GtkScrolledWindow(propagate-natural-height=true, propagate-natural-width=true, vscrollbar-policy=never, hscrollbar-policy=never)`. This provides the vadjustment that ListView requires while propagating natural size so the outer `GtkScrolledWindow` on the sidebar handles both vertical and horizontal scrolling. Without this, GtkListView may crash or emit warnings.
...
- **Dual split-view shell**: The outer window shell uses nested `AdwOverlaySplitView`s. `workspace_split_view` hosts `LushtextSidebar` on the left, and `properties_split_view` wraps the central editor/search host with `LushtextPropertiesPanel` on the right. Both side panes normalize to a fixed quarter-width fraction when shown, both toggles live in the status bar, and breakpoints collapse the properties pane before the workspace pane while preserving the same boolean toggle model in side-by-side and overlay modes. The properties breakpoint is intentionally a bit earlier than the old threshold so the center editor column stays wide enough for restored-document infobars and other editor chrome before the right pane overlays.
