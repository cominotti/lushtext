...
└── ui/                 # GTK4/Libadwaita widgets (each folder keeps mod.rs + imp.rs)
    ├── window/          # Main window shell plus workflow modules for actions, documents, drafts, notes, search, preview, print, session persistence, and zoom
    ├── editor_page/     # Per-tab editor adapter plus bookmark/annotation projection, load/save, monitor, and in-tab search helpers
...
- **Multi-workspace sidebar**: The sidebar is a two-level orchestrator. `LushtextSidebar` keeps a fixed workspace-selector row at the top, a fixed width-preset footer at the bottom, and a scrollable list of `LushtextWorkspaceSection` widgets between them. The selector row uses a `GtkDropDown` with `All workspaces` plus one item per workspace, alongside the existing add-folder button for creating a new workspace. Selecting `All workspaces` shows every workspace section; selecting a specific workspace hides the others without mutating persisted state. Each section has its own `GtkListView` + `GtkTreeListModel` + `GtkTreeExpander` file tree. The top selector row and bottom footer stay visible while the workspace list scrolls, and the sidebar no longer exposes a horizontal scrollbar when long workspace or file names exceed the visible width. The footer hosts centered `Small`, `Comfy`, and `Large` buttons, with tooltips explaining the `20% / 30% / 40%` total-window targets that drive the left split-view preset. Clicking the top add-folder button creates a new workspace (auto-named from folder). Each section has its own add-folder button for adding more roots. Right-click on workspace header shows Rename/Unlist context menu.
...
- **Dual split-view shell**: The outer window shell uses nested `AdwOverlaySplitView`s. `workspace_split_view` hosts `LushtextSidebar` on the left, and `properties_split_view` wraps the central editor/search host with `LushtextPropertiesPanel` on the right. The left pane restores one of three total-window presets (`20%`, `30%`, `40%`) from the sidebar footer, while the right pane keeps its quarter-width target and derives its inner nested fraction from the remaining width. Both toggles live in the status bar, and breakpoints collapse the properties pane before the workspace pane while preserving the same boolean toggle model in side-by-side and overlay modes. The properties breakpoint is recalculated from the active left preset whenever the workspace pane actually consumes width so the center editor column stays wide enough for restored-document infobars and other editor chrome before the right pane overlays.
...
- **Window state persistence**: Window width, height, maximized state, workspace sidebar visibility, workspace sidebar width fraction, properties sidebar visibility, and properties sidebar width fraction are persisted via GSettings. Width/height/maximized still use `connect_notify_local` on their respective properties for incremental persistence; split-view booleans and width fractions are restored in `constructed()` and persisted from split-view notify handlers. Legacy `sidebar-position` and `sidebar-visible` keys remain only as one-shot migration inputs.
...
- **Bookmarks and annotations**: Notes are available only for saved files and persist as sidecar JSON under the app data directory. `BookmarkDocument` and `AnnotationDocument` use `DocumentSidecarIdentity` with a hash of the canonical path, so Save As starts a new note identity while in-app sidebar renames migrate existing sidecars. Open editors project bookmarks as `GtkSourceMark` gutter icons and annotations as paired `GtkTextMark` anchors plus theme-aware highlight tags. Window-level browse dialogs and export flows operate on the currently selected workspace scope (including single-file workspace entries), while unsaved tabs surface explicit status-bar feedback instead of silently creating note state.
