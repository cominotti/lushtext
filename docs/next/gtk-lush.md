...
- **`ViewportSliceBin`** (from the workspace sidebar's file tree, 2026-09-14):
  host a `GtkScrollable` child inside an outer scroller without giving up
  virtualization. Advertises the child's full content height to the outer
  scroller, allocates only the visible band, owns the child's adjustments, and
  forwards `scroll_to`/focus scrolling outward. Exists because `GtkListView`
