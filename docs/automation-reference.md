...
| <span id="atspi-anchor-editor-text-editor-for-untitled"></span>`atspi-anchor-editor-text-editor-for-untitled` | Editor | `text` | `Editor for Untitled` | Untitled fallback editor after negative load or large-file policy setup | Stable public accessibility anchor |
...
| <span id="readiness-predicate-visual-geometry-settled"></span>`readiness-predicate-visual-geometry-settled` | `visual-geometry-settled` | stable | `app-startup`, `session-restore`, `file-load`, `draft-autosave`, `preview-animation`, `workspace-sidebar-animation`, `workspace-persist`, `workspace-filter-animation`, `command-palette-index`, `workspace-search`, `editor-search`, `replace-preview`, `minimap-refresh` | GTK layout, adaptive shell state, workspace sidebar animation, minimap projection, and visual scenario blockers are settled. Sidebar animation scenarios must still prove final workspace-sidebar/editor allocation relationships before screenshot proof is accepted. |
...
| <span id="readiness-predicate-accessibility-settled"></span>`readiness-predicate-accessibility-settled` | `accessibility-settled` | stable | `app-startup`, `session-restore`, `file-load`, `draft-autosave`, `close-safety`, `save`, `preview-animation`, `workspace-sidebar-animation`, `workspace-persist`, `workspace-filter-animation`, `command-palette-index`, `workspace-search`, `editor-search`, `replace-preview`, `minimap-refresh` | Accessibility tree metadata, focus targets, row rebinding, search or preview rendering, and announcement-sensitive work are settled. Accessibility smoke uses this before AT-SPI assertions, then adds narrower scenario waits when a workflow has its own completion predicate. |
