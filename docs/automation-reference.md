...
| <span id="readiness-predicate-search-complete"></span>`readiness-predicate-search-complete` | `search-complete` | stable | `app-startup`, `editor-search`, `workspace-search`, `command-palette-search`, `replace-preview` | Editor search, workspace search, command-palette query work, and Replace All preview work are no longer pending. |
