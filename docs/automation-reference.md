...
| `scripts/run-crash-recovery-smoke.sh` | `--kill-point-binary PATH` | A LushText binary built with the `crash-kill-points` feature (default `target/crash-kill-points/debug/lushtext`, which `make crash-recovery-smoke` builds). The smoke launches it with `LUSHTEXT_KILL_AT` set to abort inside `draft-body-before-commit`, `durable-renamed-before-dirsync@draft`, and `stale-preserved-before-retire`, relaunches the ordinary binary, and requires no lost work; results go to `assertions/kill-points.json` and per-scenario metadata under `kill-points/`. |
...
sidebar show/hide animation and belongs to `WFR-SHELL-GEOMETRY`, not to this row.
...
| <span id="action-app-review-preserved-drafts"></span>`action-app-review-preserved-drafts` | `app.review-preserved-drafts` | Review Preserved Drafts | `none` | `none` | `exported` | `contextual-user-command` | `app` | command-palette, dbus-action | Requires an active window; opens Preferences > Data at Preserved Drafts and deletes nothing. | unit, widget |
...
| <span id="action-win-toggle-command-palette"></span>`action-win-toggle-command-palette` | `win.toggle-command-palette` | Command Palette | `none` | `none` | `exported` | `stable-user-command` | `window/palette_shell` | keyboard-shortcut, dbus-action | Always enabled. | unit, widget |
| <span id="action-win-set-command-palette-query"></span>`action-win-set-command-palette-query` | `win.set-command-palette-query` | Set Command Palette Query | `string` | `none` | `exported` | `contextual-user-command` | `window/palette_shell` | dbus-action | Requires a visible command palette. | unit, widget |
| <span id="action-win-set-command-palette-mode"></span>`action-win-set-command-palette-mode` | `win.set-command-palette-mode` | Set Command Palette Mode | `string` | `none` | `exported` | `contextual-user-command` | `window/palette_shell` | dbus-action | Requires a visible command palette; accepts all, files, notes, or commands. | unit, widget |
...
| <span id="action-win-toggle-tab-pinned"></span>`action-win-toggle-tab-pinned` | `win.toggle-tab-pinned` | Pin or Unpin Tab | `none` | `none` | `exported` | `contextual-user-command` | `window/tab_strip` | tab-context-menu, dbus-action | Requires a tab context-menu target. | unit, widget |
| <span id="action-win-close-tabs-right"></span>`action-win-close-tabs-right` | `win.close-tabs-right` | Close All Tabs to the Right | `none` | `none` | `exported` | `contextual-user-command` | `window/tab_strip` | tab-context-menu, dbus-action | Requires a tab context-menu target; normal close safety still applies. | unit, widget |
| <span id="action-win-close-other-tabs"></span>`action-win-close-other-tabs` | `win.close-other-tabs` | Close Other Tabs | `none` | `none` | `exported` | `contextual-user-command` | `window/tab_strip` | tab-context-menu, dbus-action | Requires a tab context-menu target; normal close safety still applies. | unit, widget |
| <span id="action-win-move-tab-left"></span>`action-win-move-tab-left` | `win.move-tab-left` | Move Tab Left | `none` | `none` | `exported` | `contextual-user-command` | `window/tab_strip` | tab-context-menu, dbus-action | Requires a movable tab context-menu target. | unit, widget |
| <span id="action-win-move-tab-right"></span>`action-win-move-tab-right` | `win.move-tab-right` | Move Tab Right | `none` | `none` | `exported` | `contextual-user-command` | `window/tab_strip` | tab-context-menu, dbus-action | Requires a movable tab context-menu target. | unit, widget |
