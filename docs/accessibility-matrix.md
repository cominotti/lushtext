...
| A11Y-PREFERENCES-DATA-SET-ASIDE | Preferences Data page Preserved Drafts group; `no-context`, `representative`, `destructive` | The group is absent when nothing is set aside. Each row names the preserved draft's original file (or `Untitled document` / `Unknown file`), when it was kept, and its size, with row position metadata; the `Open` and icon-only `Delete` buttons carry labels naming the draft (`Open preserved draft of …`, `Delete preserved draft of …`). Delete asks through a destructive `AdwAlertDialog` whose default and close response is Cancel. | Preferences > Data, Tab to a row's Open / Delete, Enter; Escape cancels the delete confirmation. | An Open failure announces on the alert lane; opening and deleting are otherwise visible results. | Delete is an icon plus a named label and tooltip, never color-only. | existing: widget `preferences::test_data_page_hides_preserved_drafts_when_none_are_set_aside`, `test_data_page_lists_preserved_drafts_newest_first_and_deletes_one`, `test_data_page_opens_a_preserved_draft_in_a_new_untitled_tab_and_keeps_it`; needed: an accessibility smoke case with a seeded set-aside fixture. | Orca reads the draft's file, time, and size, and the Open/Delete labels; never the preserved body. | `ui/preferences/data_page.rs`, `services/draft_service/set_aside.rs`; privacy owner: the drafts journal. |
