...
| A11Y-MARKDOWN-REPRESENTATIVE | Markdown preview; `representative` | Preview region, read-only state, rendered text, code/table/image/link/fallback widgets, and scroll area expose useful labels/roles. Pending, complete, complete-with-omissions (`Simplified`), limited, failed, and cancelled generations expose explicit descriptions instead of silently partial content; `Simplified` reports the user-visible omission count once at the terminal, not per marker, and a block the preview already replaces with its in-place summary widget stays `Complete`. Omission markers have two affordances by design: the marker replacing one **table row** is a real accessible object (a full-width cell inside the table's `Table` role, labelled with the bare marker text rather than the `Table cell ...` data-cell prefix, and excluded from the table's announced source-row count), while the markers for a list item, quoted paragraph, definition body, code-block line, and a top-level omission past the placeholder-widget cap are plain preview buffer text with no AT-SPI anchor of their own, reachable through the preview's text interface in reading order at the replaced content's position. A top-level omission within the cap reuses the existing labelled group fallback widget. | `Alt+P`, side-by-side action, `Tab`, scroll keys, `Escape` when transient controls are present. | Planning, projection, and image work are bounded and remain in readiness until the current terminal. | Text and embedded widgets readable under opacity, dark/high contrast, and large text. | existing: accessibility `markdown-preview`; visual `markdown-preview`, `markdown-preview-side-by-side`; widget proofs cover dense multi-slice, limited, failed, stale, flood, and oversized-image states; AT-SPI smoke asserts preview text-interface plus code, table, and image fallback anchors. Added by `continue-markdown-preview-past-oversized-blocks`: widget proofs in `crates/lushtext/tests/widget/markdown_preview.rs` for the `Simplified` terminal copy and description, in-container markers for a **table row** and a **loose list item**, and the `Complete`-with-no-marker outcome for carried-embed blocks; accessibility smoke case `markdown-preview-omissions` asserts the table-row marker as a labelled AT-SPI cell plus its two surviving sibling cells, and the list-item marker through **one** text-interface assertion. The **quoted-paragraph**, **definition-body**, and **code-block-line** markers have unit coverage of their unit-naming projection only (`enclosing_unit_name_reads_the_live_continuation` in `ui/markdown_preview/continuation.rs`) and no widget or smoke assertion; no new AT-SPI anchor was added for buffer-text markers. | `Markdown preview`; Orca should read rendered fixture text and controls where exposed. | `ui/markdown_preview`; text-interface owner: preview widget. |
...
| A11Y-WORKSPACE-REPRESENTATIVE | File tree; `representative` | Tree rows expose row label, type, expanded/selected/current state, position metadata, and safe path snippets. | Arrow keys, Enter, Space for peek, context menu key/`Shift+F10`, refresh button. | Open/refresh status bounded. An attention-moment refresh that corrects a collapsed directory's frozen empty-or-not hint changes that row's expander availability and `Focus Folder` enablement **silently**: the affordance update emits no announcement, and focus plus selection stay on the same path. Announcing it would interrupt a screen-reader user for a change they did not initiate. | Selected row/focus visible; directory/file/icon state not color-only. | existing: accessibility `workspace-tree`; visual `workspace-representative`, `workspace-refresh`. widget `attention_refresh::test_collapsed_empty_directory_stops_claiming_empty_after_attention` (hint correction plus preserved tree state). | Orca reports folder/file rows, expansion, selection, and refresh. | `ui/sidebar/workspace_section`; row metadata apply/clear present and must stay covered. |
| A11Y-WORKSPACE-DENSE-DEEP | File tree; `dense/awkward` | Deep paths, long names, file/folder row recycling, focused folder affordance, and clipped labels have bounded descriptions; rows beyond `GtkListView`'s 200-widget realized cap render and expose the same metadata as early rows. | Arrow keys, focus-folder command/context menu, refresh, open. | Dense refresh/search count announcements remain bounded. | No horizontal scrollbar; headers/actions remain visible; deep rows ellipsize or clip intentionally; every tree-model row renders when scrolled into view (no blank band after ~205 rows). | existing: accessibility `workspace-tree-dense-constrained`, `workspace-tree-deep-expanded`; visual `workspace-dense-awkward`, `workspace-constrained` (a screenshot case for a 300-entry directory is deferred: the headless capture harness cannot expand a folder or scroll the tree without an automation-only reveal action, see the `fix-sidebar-large-directory-rendering` tasks); widget `workspace_tree_virtualization::*` (rendered-row completeness, boundary counts, keyboard traversal, late-row accessibility parity); needed: focus-folder a-smoke. | Orca should receive row name/type and not stale previous row metadata after recycling, including for rows past the realized cap. | `ui/sidebar/workspace_section` (`file_tree_slice`, a `gtk_lush_widgets::ViewportSliceBin`); row unbind audit required by policy. |
...
| A11Y-WORKSPACE-SEARCH-UNDO-JOURNAL-ALERT | Workspace search Replace All undo journal; `error` | A failed undo-journal write is named as a durability warning, not a generic status change: the message states that undo may not survive a restart. | `Ctrl+Shift+F`, Replace All, Undo; no keyboard path of its own, because the alert is workflow-emitted. | High-priority `AnnouncementLane::Alert` via `announce_workflow_update("workspace-search-alert")`, emitted for any `Warning`/`Error` panel message so a screen-reader user need not notice the status lane changed colour. Informational panel messages such as "Search saved as ..." stay silent. | Status lane renders the message on `@warning_color`, not `@accent_color`. | existing: widget proof that the journal disk-write failure path publishes through the panel message callback with `NotificationSeverity::Warning`; needed: AT-SPI smoke for the announcement, which requires injecting a durable-write failure into a live session. | Orca announces the durability warning at alert priority; the message text is bounded and carries no file content. | `ui/search_panel/journal.rs`, `ui/window/search.rs`; audit status: announcement wired and severity-gated, live AT-SPI proof outstanding. |
...
| A11Y-ENCODING-DIALOGS | Encoding, save-encoding, reopen-encoding, line-ending, invisible-character, and file-health dialogs; `representative`, `no-context`, `dense/awkward` | Each `AdwAlertDialog` carries a stable accessible name taken from its visible heading. Grouped `AdwActionRow` choices carry `Radio` role and expose the **current** option as accessible **checked** state, not by glyph alone (`Selected` is dropped by `GtkListBoxRow`'s own accessible context, so `Radio` + `Checked` is both the treatment that lands and the ARIA-correct one for one-of-many); rows that open a further surface expose **has-popup**; the two suffix images (`object-select-symbolic`, `go-next-symbolic`) are `Presentation` so they are not announced as bare images. Row titles and subtitles carry the option's meaning, and the empty file-health state has readable copy. | `win.show-encoding-controls`, `win.show-line-ending-controls`, `win.show-file-health`, status-bar encoding/line-ending controls, invisible-characters shortcut; `Tab`/arrows within groups, `Escape` to dismiss. | Policy changes report through the bounded status lane; no per-row announcement while traversing options. | Long encoding and line-ending subtitles wrap rather than truncate (`title_lines(0)` / `subtitle_lines(0)`); the current option is distinguishable without relying on the checkmark glyph; explicit inner content padding per Dialog Text Surface Padding. | existing: `test_encoding_dialog_rows_expose_checked_and_has_popup_metadata` in `tests/widget/window.rs`; accessibility `doc-surfaces`; needed: AT-SPI fixture for the encoding dialog tree. | Orca announces the option list, states which option is checked, and reports that an action row opens a further dialog. | `ui/window/encoding/dialogs.rs` routes all metadata through `crate::ui::accessibility`; decisions live in `ui/window/encoding/policy.rs`; matrix owner: `WFR-ENCODING`. |
...
| A11Y-WORKSPACE-VISIBILITY-CONTROLS | Hidden-files view mode and excluded-names editor; `representative`, `no-context`, `error` | The main-menu check item, the Preferences switch, each excluded-name row and its remove button, the add entry, the reset control, the empty state, and the search panel's `Hidden files` toggle carry labels and descriptions; the add entry exposes an invalid state plus the rejection reason. | `Ctrl+Shift+H`, main menu, Preferences > Workspace, Tab/Enter in the add entry, Tab to the search panel toggle. | A visibility toggle refreshes the tree silently; no manual-refresh message or announcement. | Excluded names, the empty state, and the add-entry error are not color-only. | existing: accessibility `preferences`, `workspace-search`; widget: `workspace_entry_visibility`. | Orca reads the check item state, each excluded-name row, the remove and reset controls, and the add-entry rejection. | `ui/preferences/excluded_names.rs`, `ui/search_panel`, `app.rs`; static rows use helper metadata. |
...
| A11Y-PREFERENCES-DATA-SET-ASIDE | Preferences Data page Preserved Drafts group; `no-context`, `representative`, `dense`, `destructive`, `constrained/compact` | The group is absent when nothing is set aside. Its first row is a summary whose name is the count (`37 preserved drafts`, or `At least …` when the scan budget was reached) and whose description gives the total size, `over the suggested limit` past the soft bound, and `showing the newest 256 of N` when not every body is listed. The second row is the `Delete All Preserved Drafts…` button row, labelled and described (it asks first). The per-body rows sit in their own bounded list (`Preserved drafts`, role list), so a dense area scrolls inside it and the summary and bulk action stay reachable. Each row names the preserved draft's original file (or `Untitled document` / `Unknown file`), when it was kept, and its size, with row position metadata; the `Open` and icon-only `Delete` buttons carry labels naming the draft (`Open preserved draft of …`, `Delete preserved draft of …`). Per-row Delete and Delete All each ask through a destructive `AdwAlertDialog` whose default and close response is Cancel; the Delete All body states the exact count and total size it deletes and that drafts preserved after it opened are kept. | Preferences > Data, Tab to the summary, the Delete All row, or a row's Open / Delete, Enter; Escape cancels either confirmation. | An Open failure, and a confirmed per-row Delete that deleted nothing (the draft changed or went away first, or could not be removed), announce on the alert lane; a finished Delete All announces its result (deleted, kept because changed, failed) on the status lane. | Delete is an icon plus a named label and tooltip, and Delete All uses the destructive style plus its text, never color-only. | existing: widget `preferences::test_data_page_hides_preserved_drafts_when_none_are_set_aside`, `test_data_page_lists_preserved_drafts_newest_first_and_deletes_one`, `test_data_page_opens_a_preserved_draft_in_a_new_untitled_tab_and_keeps_it`, `test_data_page_summarizes_preserved_drafts_over_the_soft_bound`, `test_data_page_states_when_only_the_newest_preserved_drafts_are_listed` (narrow window, bounded list), `test_delete_all_preserved_drafts_states_count_and_size_and_cancel_deletes_nothing`, `test_delete_all_preserved_drafts_removes_exactly_the_confirmed_set`; needed: an accessibility smoke case with a seeded set-aside fixture. | Orca reads the summary's count, size, and bound state, the draft's file, time, and size, and the Open/Delete/Delete All labels; never the preserved body. | `ui/preferences/preserved_drafts.rs`, `services/draft_service/set_aside.rs`, `services/draft_service/set_aside_retention.rs`; privacy owner: the drafts journal. |
| A11Y-PREFERENCES-DATA-SET-ASIDE-REVIEW | Soft-bound review notice and `app.review-preserved-drafts`; `representative`, `recovery` | When the set-aside area passes the soft bound (100 bodies or 256 MiB), one window status warning reads `LushText is keeping N preserved drafts (size). Review them with Review Preserved Drafts.` at most once per launch, again only after material growth, and never after the review action ran. `Review Preserved Drafts` is a command-palette command and D-Bus action that opens Preferences on the Data page and moves focus to the Preserved Drafts summary row. | Ctrl+Shift+P, type `Review Preserved Drafts`, Enter; focus lands on the summary row. | The notice uses the status-bar warning lane and is not repeated. | The notice is text in the status bar with the warning style, never color-only. | existing: widget `preferences::test_preserved_drafts_over_the_bound_notify_once_and_the_review_action_opens_them`; needed: an accessibility smoke case that runs the action. | Orca reads the status warning and then the focused summary row; no body text. | `ui/window/set_aside_review.rs`, `app.rs`, `services/draft_service/set_aside_retention.rs`; privacy owner: the drafts journal. |
...
| A11Y-RECOVERY-STARTUP | Draft/session/recovery startup surfaces; `recovery`, `error` | Recovery diagnostics, restored tabs, failed metadata, quarantine/repair actions, and startup alerts expose bounded names; the stale-draft alert ("Draft Not Restored") names where the edits were kept and, for Local History, exposes a named "Show in Local History" action button. | Startup, Tab through alerts, repair/skip actions, Escape when safe. | Recovery/repair status bounded. | Recovery warning not color-only and does not block main chrome invisibly. | existing: visual `recovery-startup`; needed: accessibility smoke/manual Orca for recovery workflow. | Orca reports recovery status and available actions without private draft body. | `ui/window/startup`, recovery services; privacy owner: recovery metadata. |
