...
| Quantity | Count |
| --- | --- |
| `*.rs` files under `crates/lushtext-core/src` | **308** |
...
| **attributable to a matrix row or to the list above** | **302** |
...
| --- | --- | --- |
| 23 | `model/**` | the domain rows and the workflows that own each value type; `model/editor_memory.rs` and `model/migration_ledger.rs` are the two `exempt` / `cross-cutting` rows |
...
| 6 | `ui/*.rs` | `accessibility.rs` and `theme.rs` in the list above; `automation.rs` is `WFR-AUTOMATION-SPINE`; `buffer_snapshot.rs` is `WFR-BUFFER-SNAPSHOT`; `plain_disposal.rs` is `WFR-PLAIN-DISPOSAL`; `mod.rs` is the crate's `ui` module root |
...
| 3 / 2 | `ui/preferences/**`, `ui/properties_panel/**` | the list above |
