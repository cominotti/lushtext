...
- `superseded`: the row was **replaced** by other rows rather than migrated,
  exempted, or shared. It is terminal, and its cell names every replacement row
  so the history stays reachable. Added in slot 7b for `WFR-SHELL-LAYOUT`, which
  slot 7a resolved was never one workflow: none of the five labels above
  describes a replaced row, because `exempt` and `cross-cutting` both claim the
  code is deliberately unmigrated and `deferred` is transitional. A superseded
  row is **retained, never deleted** — deleting it would leave the programme
  record's ledger naming a row the matrix no longer has.

`pending`, `deferred`, and `partially-conforming` are **transitional**: no row
may hold one at programme close. They remain in `KNOWN_STATUS_LABELS` rather
than being removed, because the gate must keep failing *informatively* on them
(and on an unknown label) instead of silently exempting the row — removing a
label makes it unrecognized, which is a finding, but a less legible one than
"this transitional status is not terminal".
...
| WFR-MARKDOWN-PREVIEW | Markdown preview render, images, footnotes, tables | 11 files, 11,274 lines (ui 8,334 / services 2,940); re-measured after `continue-markdown-preview-past-oversized-blocks` added `ui/markdown_preview/continuation.rs` (1,170) and `text_flow.rs` (265) while `mod.rs` fell 2,541 → 1,985, and grew `services/markdown_render.rs` 556 → 2,940, of which ~1,700 are co-located `#[cfg(test)]` planner tests. The `ui` subtotal spans `ui/markdown_preview/**` (7,762) plus `ui/window/preview.rs` (572). **Re-derived in slot 7: the `ui` subtotal is 8,345 physical / 7,056 production** (`ui/markdown_preview/**` 7,773 / 6,500 plus `ui/window/preview.rs` 572 / 556) — the cell is low by 11 physical, `continuation.rs` measures 1,181 rather than 1,170, and the figure is **raw**, so it overstates the reader's burden by 1,289 lines (18%). **Re-derived by `fix-markdown-preview-stale-after-load`, which was owed regardless of its own edit because the slot-7 figure was already stale: 19 files, 12,132 physical / 8,872 production** — `ui/markdown_preview/**` **17 files, 8,279 / 6,984** (the slot-7 figure predates slot 7a's role files), `ui/window/preview.rs` **673 / 657** (the recorded 572 / 556 had drifted to 586 / 570 before this change; the change added the content-republished preparing placeholder, gated on the editor page's `content_is_installing`, and the frame-capped deferred `show-sidebar` that avoids Libadwaita's `0 / 0` spring velocity), and `services/markdown_render.rs` **3,180 / 1,231**. The `ui` subtotal is **8,952 / 7,641**. `ui/window/preview.rs` stays a called presentation surface: the installing predicate lives on the editor page, which owns the state it reads, and the change added no `*_for_test` seam. **Re-derived by `extend-kani-to-pure-policies`, which moved the side-by-side preview-width clamp (`clamped_preview_width`, `preferred_preview_width`, `PREVIEW_MIN_WIDTH_SP`, `PREVIEW_DEFAULT_WIDTH_SP`, and `PREVIEW_MAX_WIDTH_FRACTION`, which became the integer `PREVIEW_MAX_WIDTH_DIVISOR`) out of `ui/window/preview.rs` into `ui/markdown_preview/policy.rs`.** Counted on both sides with one tool that excludes `#[cfg(test)]` and `#[cfg(kani)]` items (it reads the pre-change tree as **8,952 / 7,584**, so its production figure is 57 below the 7,641 above, a method difference rather than drift): the `ui` subtotal is **9,002 physical / 7,622 production** across the same 18 files, `ui/markdown_preview/policy.rs` **1,188 / 691 → 1,253 / 744**, `ui/window/preview.rs` **673 / 657 → 655 / 639**, and `mod.rs` **244 → 247** (one re-export, inside the 370 facade budget). The new `policy/kani_proofs.rs` (84 lines) is verification code under `cfg(kani)` and is counted in neither figure | `Alt+P`, `win.toggle-preview-mode`, side-by-side action, buffer changed | none in `model/`. `ui/markdown_preview/policy.rs` now also owns the preview-width clamp, integer-only since the move (whole pixels in and out; `ui/window/preview.rs` converts to `f64` once at the split view), proved by Kani (`policy/kani_proofs.rs`): at least the 1 sp floor, at most one third of the available width from 3 sp up, the floor below 3 sp (a stated exception kept as the `should_panic` harness `preview_width_is_not_always_a_third`), an in-band preference kept exactly, and monotone in the preference. **Pure-policy consumers, re-derived: 2 owning workflows** — this row (`ui/window/preview.rs`, a called presentation surface, and `ui/window/imp.rs`'s initial preferred width) and `WFR-SHELL-GEOMETRY` (`ui/window/geometry/execution.rs` seeds the split view's bounds with `PREVIEW_MIN_WIDTH_SP` before the first width sync); before the move the clamp sat in a GTK module and had **0** mutants in scope, so the mutation count of `policy.rs` rising from 177 is a **gain from zero**, not parity | **13/3/3/2** = 21 gated declarations, 56 gate sites. Total and sites are **exact**; the **kind split is wrong** and was re-derived in slot 7 from the pre-slot-7 `12/4/3/2` — inspection low by one, configuration high by one, because the census missed **both** seams in `ui/window/preview.rs` (`preview_transition_pending_for_test`, inspection; `set_preview_transition_pending_for_test`, actuation). Also **10 test-only statics, not 3**: three timing overrides plus seven observation counters, all at `mod.rs:211`–`:229`, and all seven are equally in scope for the row's single test policy value | exists: `MarkdownRenderSession::is_current(generation)`; plus the planner/projector batch seam `MarkdownCarrySignature` + `MarkdownOpenContainer` (expected/open containers per batch, chained across turns), `MarkdownBlockOmission` (omission reason, scope, and unretained charge crossing the same seam), and the projector-side `MarkdownProjectionContinuation` + `ContinuationBreach` that holds and validates it | **none in this row's adapter.** `MarkdownImageAdmissionSnapshot` is **not declared in `ui/markdown_preview/**`** — it is `services/markdown_render.rs:158`, a services-owned type — so the pre-slot-7 `partial:` claim named a module this row does not own. All 13 inspection seams return bare tuples, so the consolidation obligation is **larger** than the cell implied, not smaller. Note also that **four of the six seam value objects live in `services/markdown_render.rs`**; only `MarkdownProjectionContinuation`/`ContinuationBreach` are adapter-side | tier-2 | 7a | migrated |
| WFR-MINIMAP | Minimap strip, markers, native source map geometry | **MIGRATED by slot 6. Current size: 11 files, 3,414 production lines** (non-`#[cfg(test)]` only), all under `ui/editor_page/minimap/`. **Both census figures were raw, not production, and the size cell was wrong by 34%**: it read `2 files, 3,965 lines (ui 3,779 / model 186)`, which is `wc -l`. Re-derived before the move with the unit named: the pre-move `minimap.rs` **2,509 production / 3,779 raw**, the pre-move `minimap_analysis.rs` **120 production / 186 raw**, row **2,629 production across 2 files**. The post-migration **+785 production** is role narration, the facade, `evidence.rs` (216), and `widgets.rs` (59); no behavior moved. Per-file: `policy.rs` 944 production / 2,454 raw (the raw figure exceeds the ~1000 target because 1,510 lines are co-located pure tests), `projection_execution.rs` 847, `mod.rs` 366, `reflow_execution.rs` 252, `watch.rs` 228, `evidence.rs` 216, `admission.rs` 193, `analysis_execution.rs` 142, `retirement.rs` 116, `widgets.rs` 59, `test_policy.rs` 55. **Four of those figures were re-derived after the post-review simplification pass** and are net +15 against the first derivation (`projection_execution.rs` 832 → 847, `analysis_execution.rs` 135 → 142, `watch.rs` 233 → 228, `retirement.rs` 118 → 116): eight verbatim coordinate-conversion closures collapsed into one named `source_map_widget_y`, the thrice-spelled slice freshness guard became one named `minimap_analysis_is_current` predicate, and the lines those removed were spent on doc comments for two look-alike iter helpers and a no-op clamp. `mod.rs` is unchanged at **366**, inside the 370 facade budget. `ui/editor_page/imp.rs` is **not** counted here: `MinimapState` is a called presentation surface shared with the editor page, and pooling it would double-count what `WFR-DOCUMENT-SAVE` and `WFR-DOCUMENT-LOAD` already record as the page's own state | `win.toggle-minimap`, `Ctrl+Shift+M`, buffer edits, search sessions, bookmarks, scroll adjustments, style changes, and sidebar/properties width reflow; plus modified-line marks driven by four other workflows (load, save, local-history restore, draft restore) | `crates/lushtext-core/src/ui/editor_page/minimap/policy.rs` — **944 production / 2,454 raw**. Owns every threshold and clamp (the 8px marker strip, the 13px native-slider CSS outset, the 0.20 wide-editor ratio, the exact 2 MiB wrapped-layout budget, the three marker caps, the three `Duration`s), the marker-bounds and projected-bounds fitting, the native-slider estimate, the availability priority order, the lane widths/positions and the light/dark palette, the modified-line sampling, and the **relocated** GTK-free content accumulator. **The relocation is DONE with exact mutant-by-mutant parity**: the pre-move `minimap_analysis.rs` moved here as the row's single owning workflow — the only destination that keeps it inside the `ui/**/policy.rs` mutation scope — at **21 generated / 19 caught / 2 missed / 0 unviable before, identical after**, the same two survivors on the same function, not one generated mutant changed. Those two inherited survivors were then **triaged to 0**. **Extraction gain is reported separately** because it has no before-count and cannot fail: **404 mutants of newly-extracted pure policy that were not in the deterministic lane before, 394 caught on the first run**. **Survivor triage went to step 3 of the documented order rather than stopping at step 4**: the four `fit_projected_bounds` mutants the stale `line:column` anchors had been written for were confirmed still generated and still unprotected, and three of them are genuinely equivalent — but symbol-scoping them would have swallowed three mutants the run had **caught**, in that function's unrelated guards, which is a weakening the amendment forbids. So the min-height expansion block that was **duplicated verbatim** between `fit_marker_bounds` and `fit_projected_bounds` was extracted into one `expanded_to_min_height` helper, which made the equivalence claim expressible narrowly (one mutant per entry) and then exposed `.min(upper - lower)` as dead code — deleting a fourth survivor instead of excluding it. **Final confirming run: 412 generated / 406 caught / 0 missed / 6 unviable — survivors triaged to zero.** **Kani (`extend-kani-to-pure-policies`)**: (design D10 later made the wide-editor ratio an exact integer comparison, `map * 5 > editor`, and moved `MinimapMarkerBounds::height`'s subtraction into its one cairo caller in `projection_execution.rs`, leaving 10 functions admitted by a function-level `expect` under the recorded ceiling of the convention's whole-pixel table; `policy.rs` is 562 lines before its first `#[cfg(test)]` item, from 566) the fit functions are proved in the harness module `policy/kani_proofs.rs` (verification code under `cfg(kani)`, outside the mutation scope and the size cell) never to panic or return a non-finite coordinate for any `f64`, and to stay inside the rendered-content band on whole pixels. The no-panic harness found that `fit_native_slider_to_source_map_bounds` returned an infinite height for a source map near `f64::MAX` tall; fixed failing-first with two guards in `policy.rs` (its production figure rises accordingly, and the new mutants are reported by that change) **The census cell read `1 consumer` and was low by one**: the module has **two** consumers — the in-crate adapter and the external `crates/lushtext-core/benches/benchmarks.rs` target — but **one owning workflow**, and eligibility is counted in owning workflows, so the conclusion stands. The bench keeps reach through a precisely scoped `pub use` in `editor_page/mod.rs`; `mod minimap;` stays private, matching `load/`, `save/`, and `buffer_replacement/` | **RETIRED by slot 6: 11 fns / 21 gate sites → 2 fns / 15 gate sites.** Seven inspection seams plus the eleven-field `MinimapAnalysisSnapshot` consolidated into `MinimapEvidence`, with 37 widget-test call sites rewritten. **The census cell read `9/1/1/0 = 11 fns, 16 sites` and under-counted the sites by 5**: sixteen `#[cfg(feature = "test-utils")]` gate sites were in the pre-move `minimap.rs` and the census stopped at the filename, but **five more are `MinimapState` fields in `ui/editor_page/imp.rs`** (`analysis_slices`, `analysis_chars_per_slice_high_water`, `analysis_cancellations`, `analysis_terminals`, `analysis_after_slice_hook`), every one of them read by the retired snapshot accessor. A row's seam census follows the row's state, not the row's filename. **Zero new actuation seams**, and the row's two existing ones are dispositioned rather than carried: `mark_minimap_refresh_pending_for_test` **retired** (its three readiness call sites now drive a real buffer edit through the production `insert-text` path), and `set_after_minimap_analysis_slice_hook_for_test` **kept and justified at its definition** in `test_policy.rs`, because the window it injects into is between two slices of one `idle_add_local` turn and no production drive can enter it. `_for_benchmark` seams: **0**, confirmed | exists: `MinimapAnalysisSession` (`{generation, lifetime}`), audited and unchanged. **done: `MinimapProjectionSpace` and `MarkerProjectionSpace`** promoted from private structs in the adapter to reified pure seams in `policy.rs`, each crossing three or more function boundaries; they are what let the fitting arithmetic take no GTK type and make passing a marker-strip coordinate where a target coordinate is expected a type error. **Dropped: the adjustment-facts candidate.** Implementation could not qualify it — `fitting_source_map_page_size` takes two pairs from **two different** adjustments and `finite_adjustment_distance_from_lower` takes a pair, so it crosses neither limb of the seam rule — and `MinimapAdjustmentDiagnostics` already reifies the same five facts, so a second type would have been a parallel shape rather than a seam | **done: `crates/lushtext-core/src/ui/editor_page/minimap/evidence.rs`** — 216 production, one `MinimapEvidence` value of **20** fields read by one accessor. All three mandated proofs are **driven, not asserted**, and the disposal proof **found a real defect in the surface it was written for**: `wrapped_layout_analysis_required` reached `source_view()`, a panicking `TemplateChild` accessor, so a teardown read crashed instead of answering; the field now probes with `try_get()` first. The reentrancy proof reads after each mutating operation, never during. The inertness proof compares all four analysis metrics, both generations, and both pending flags across five repeated reads at two stages | tier-2 logic, pixel-verified proof | 6 | **migrated** — slot 6 (`migrate-minimap-workflow-readability`) |
| WFR-BUFFER-SNAPSHOT | Bounded GTK buffer text capture | 1 file, 1,149 lines (ui) | called by save, draft autosave, encoding analysis, preview, local history | `model/plain_disposal.rs` is consumed through `plain-disposal`, not owned here | 9 gated declarations / 40 `test-utils` gate sites — the densest gate-to-line ratio in `ui/`. Size re-derived as **1,149 physical / 1,084 production**; the cell stated only physical | exists: `BufferSnapshotHandle` + `BufferSnapshotPayload` | **`BufferSnapshotEvidence`** — **consolidated in slot 7a** from the three parallel typed observation types the cell recorded as `partial`. They became named *components* (`BufferSnapshotSessionEvidence`, `BufferSnapshotCaptureMetrics`, `BufferSnapshotHandoffCounters`), and the five capture-metric fields that had been declared in **two** of them are now declared once. All three surface proofs discharged, the disposal one against a real released session. `BufferSnapshotTestMutation` is classified as a **mutation injector** — a configuration/actuation seam, not a fourth observation path — and `char_count_requires_chunked_snapshot` stays a shared limit, unmoved and unforked | tier-2 | 7a | cross-cutting — surface obligations **discharged** |
| WFR-PLAIN-DISPOSAL | Off-GTK retirement of large owned payloads | **2 files, 2,332 physical / 1,905 production**: `ui/plain_disposal.rs` 1,621 / 1,422 and `model/plain_disposal.rs` 711 / 483. Re-derived in slot 7b under a stated predicate — physical lines minus each file's trailing `#[cfg(test)] mod tests` block, counted from the `#[cfg(test)]` attribute line to EOF. Both baseline corrections were **downward** by one (`1,344 -> 1,343` and `~465 -> 464`), giving a baseline of 1,343 + 464 = **1,807**; production then **grew by 98** in this change, which is the expected direction for a consolidation that adds one typed surface while removing four accessor bodies. The figures were re-derived once more after the review pass, and both halves had moved: the `ui` half by +7 and the `model` half — recorded as *unchanged* before the review — by **+19**, because `is_quiesced()`'s definition moved into the domain `PlainDisposalSnapshot`. A size cell measured before the last edit is a stale cell, which is the lesson this change records for its facades as well | called by 21 files across 10 workflows | `model/plain_disposal.rs` (1 consumer file, but its consumer is this module's own adapter → cross-cutting, stays) | **8 gated declarations before, partitioning as 4 observation + 4 actuation** — not the 5 + 3 the design assumed. `aggregate_pressure_evidence_for_test` is **not** an observation accessor: it is a 185-line fixture driver that spawns worker threads, blocks on a `Condvar`, saturates the lane, and contains ten assertions. Folding it in would have made *reading* the surface spawn threads and mutate every counter the surface reports, breaking both the non-materialization and reentrancy proofs; the exclusion is stated in the surface's own module doc. The four observation accessors are retired into one surface, leaving **4** `*_for_test` declarations, all actuation. Gate sites: `ui/plain_disposal.rs` now carries **27** `cfg(feature = "test-utils")` and **4** `cfg(any(test, feature = "test-utils"))` (two more were stacked *dead* beneath a narrower `test-utils` attribute and are collapsed); `model/plain_disposal.rs` still carries **0** of either | exists: `DisposalOwned<T>` + `DisposalPermit` | **`PlainDisposalEvidence` via `plain_disposal_evidence()`**, one accessor with the ordinary and progress lanes as **named components** rather than two top-level accessors. It also names `is_quiesced()` once, a predicate that previously existed only as an open-coded `running_jobs == 0 && queued_jobs == 0` re-derived at every call site, so every proof carried its own definition of "quiet". **Visibility: already narrowest**, recorded with its measurement — the reader set is 35 call sites across 5 files, every one in the `lushtext` crate, so `pub` under a `test-utils` gate is the narrowest visibility that compiles. Slot 7a's instruction to narrow `DisposalPressureEvidence` *from* `pub` is **unexecutable as written**; executing it would break the widget lane and call it a narrowing | tier-3 | 7b | cross-cutting — **surface obligations discharged** in slot 7b, matching `WFR-BUFFER-SNAPSHOT`'s resolved form. No facade, no coordination role names, no `policy.rs`: the row is one admission mechanism, not a workflow. All three surface proofs are discharged and driven, sharing one `quiesce_lanes(budget)` helper that drains to a terminal and confirms zero running and zero queued **through the surface itself** — necessary because the lane's counters are process-wide atomics advanced by worker threads, so read-to-read identity is not a property of the reader's control flow. Under an outstanding hold the assertion is **monotonicity**, not equality. No shared limit moved or forked, and confirming that surfaced a real defect fixed in-stream: `ui/window/session_restore/journal.rs` aliased `PROGRESS_DISPOSAL_RETAINED_BYTE_CAPACITY` as `STARTUP_PRELOAD_RESERVATION_BYTES` with **no doc comment** — a value renamed while crossing a seam — which now carries the owner and contract the rule requires. `ui/plain_disposal.rs`'s 1,422 production lines are recorded as **accepted refactor debt** in the closeout inventory rather than split blind |
| WFR-EDITOR-FIND | In-tab find and replace | **4 files, 1,043 physical / 967 production** in `ui/search_bar/**` plus `ui/editor_page/search.rs` (112); the pre-migration cell read `3 files, 824 lines (ui)`, which measured exactly | `Ctrl+F`, `Ctrl+H`, `Ctrl+G`, `Ctrl+Shift+G`, `Escape` | **relocated to `ui/search_bar/policy.rs`**: counter text, announcement wording, the invalid-query predicate, the occurrence-position floor, and the search-option vocabulary. The census said `none`; probing found five decisions, all user-visible. Gain from zero: **36 mutants** | **0 gated declarations, 0 gate sites** — re-derived and **exact** | none required — no bundle crosses two boundaries | **none, and that is the recorded conclusion**: with zero seams there is nothing to consolidate, and the row's observable state is already production API (`search_context`, `has_navigated`, `is_replace_revealed`). Adding a surface would widen the API for no reader | tier-1 | 7a | migrated |
| WFR-ENCODING | Encoding and line-ending controls | **2 files, 952 physical / 952 production lines** (ui). Re-derived in slot 7; the pre-slot-7 cell read `1 file, 907 lines (ui)` and **omitted `ui/editor_page/invisibles.rs` (45)** entirely — a missing file, not just a low number | `win.show-encoding-controls`, `win.show-line-ending-controls`, `win.show-file-health` | `model/encoding.rs` (15 consumers → domain, stays) **plus a relocated `ui/window/encoding/policy.rs`**: every word shown to the user and every selected/activatable decision, including the mixed-line-ending exception that keeps the current style activatable. The domain module was **not** forked. Gain from zero: **32 mutants** | 2 gated declarations / 4 gate sites, re-derived as exact; both are halves of **one** timing override, now the row's single `test_policy.rs` | none required — no bundle crosses two boundaries; the freshness triple is rechecked through one shared predicate | **none, and that is the recorded conclusion**: the two gated declarations were a configuration override rather than inspections of live state, and every stage's result is read through the editor's own production accessors. The write crosses into `WFR-DOCUMENT-SAVE`, which owns that seam | tier-1 | 7a | migrated |
| WFR-PRINT | Print document | **5 files, 372 physical / 300 production** in `ui/window/print/**` after migration; the pre-migration cell read `1 file, 172 lines (ui)`, which measured exactly | `win.print` | **relocated to `ui/window/print/policy.rs`**: the outcome vocabulary and the report decision. The census said `none`; probing found both, and neither had mutation coverage. Gain from zero: **3 mutants** | 1 gated declaration / 8 gate sites — a **lifecycle probe**, retained per the probe rule, and re-pointed at the row's evidence surface | none required — one synchronous request, no inversion, no generation counter | **`PrintEvidence`**, which **folded in** the former standalone `PrintDocumentSnapshot` as its `document` component rather than leaving a second typed observation path | tier-1 | 7a | migrated |
| WFR-SHELL-LAYOUT | Window shell, tabs, split views, focus mode, zoom | **18 files, 9,737 physical / 9,522 production lines** (ui), re-derived in slot 7. The pre-slot-7 cell read `19 files, 8,449 lines (ui)` with the unit unstated; 8,449 matches **no** sub-partition of the file set, so it predated substantial growth. The file count moves twice and in opposite directions: `ui/sidebar/width_preset.rs` (125) joins, per slot 5b; and **`ui/properties_panel/**` (333) leaves**, because slot 7 resolved its double attribution in favour of the [no-coordination-tier list](#surfaces-with-no-coordination-tier) — it declares 2 `pub fn`s, 0 test seams, and has exactly one production caller (`documents.rs`, inside `refresh_status_bar`), and no shell stage order calls it | `win.toggle-sidebar`, `win.toggle-properties`, `F9`, `win.toggle-focus-mode`, `win.new-tab`, tab actions, breakpoints, resize | **relocated, and the row no longer owns it.** the former ui/window/policy.rs moved to `crates/lushtext-core/src/ui/window/geometry/policy.rs` when `WFR-SHELL-GEOMETRY` took its role home in slot 7b. **Relocation parity measured from the tool on both sides: 81 mutants before, 81 after** — read at the old path from a clean worktree at `69f78b09` and at the new path in this tree, with the file byte-identical apart from visibility keywords. The cell's earlier figure of **80** was stale by one and is corrected here; slot 7a's gain-from-zero claim (0 -> 80 when `adaptive_shell.rs` was renamed into the convention) stands as its own separate figure and is not restated as parity | **48 gated declarations and 82 `test-utils` gate sites**, re-derived in slot 7 against the pre-slot-7 cell's `11 fns, 47 sites` — **low by 37 declarations, a 4.4x understatement** — plus 1 override static. **Cause, which matters more than the number:** slot 3b assigned `ui/open_popover/**` and `ui/window/recent_open.rs` to this row when it found they appeared in no row's file set, and **the seam cell was never re-derived after the assignment**. The recent-documents surface alone carries 26 declarations and 37 sites, more than every other surface in the row combined. Note also that `ui/window/mod.rs` has 11 gate sites and **0** declarations because it gates *re-exports*, so a merged "sites" metric double-counts them | **the `none` claim is contradicted by slot 7's trace**: the row's files already hold at least four unnamed freshness/identity values (`CloseSafetyEditorFingerprint` with its `close_discard_fingerprints_are_current` predicate, `CloseSavePipeline`, `GuardedFileIndexBuildOutcome`, `GuardedReplaceApplyResult`/`GuardedUndoResult`) and at least six worker completion seams | none needed | **tier-3**, re-derived in slot 7a against the pre-slot-7 cell's `tier-1`: the row's files own tab close and delete-driven close (where slot 7's confirmed data-safety defect lived), `dialogs.rs` owns the confirmed-close coordination contract, and `startup_data.rs` gates the format-upgrade preflight. A row that owns tab close is not tier-1 | 7b | superseded — **by the seven rows below**, in slot 7b. The label is new: slot 7a resolved this was never one workflow, and none of `pending`, `migrated`, `exempt`, `deferred`, `cross-cutting`, or `partially-conforming` describes a row that was **replaced** — `exempt` and `cross-cutting` both claim the code is deliberately unmigrated, and `deferred` is transitional. `superseded` was added to `KNOWN_STATUS_LABELS`, `SETTLED_STATUSES`, and `TERMINAL_NON_MIGRATING_STATUSES` in `scripts/check-workflow-boundaries.py`, with its own self-test arm proved by deliberate red. **Superseded by:** `WFR-TRANSIENT-DISMISSAL`, `WFR-FOCUS-MODE`, `WFR-EDITOR-MEMORY-EVICTION`, `WFR-RECENT-DOCUMENTS`, `WFR-SHELL-GEOMETRY`, `WFR-TAB-STRIP`, `WFR-STARTUP-PREFLIGHT`. The row is retained rather than deleted so its history stays readable: its evidence and the four contested-file verdicts are in `openspec/changes/archive/2026-08-28-complete-residual-workflow-readability/tasks.md` Appendix A.4 |
| WFR-TRANSIENT-DISMISSAL | Escape and click-away dismissal of shell-owned transient surfaces | **4 files, 691 physical / 566 production** in `ui/window/transient_dismissal/**` (facade 199, policy 265 raw of which 125 are co-located tests, evidence 70, surfaces 157). Replaces the 202-line retired ui/window/transient_surfaces.rs. The row **grew**, which the thin-row test asks about and which is justified in slot 7b's appendix A.2: the ladder's order was previously expressible only by reading five interleaved `if` blocks and carried **zero** mutation coverage | `Escape` anywhere in the shell; a primary pointer press outside the command palette | `crates/lushtext-core/src/ui/window/transient_dismissal/policy.rs` | 0 inspection declarations before; 2 pre-existing actuation seams preserved, 0 added | **done**: `TransientSurfaceState` — five same-typed booleans, the archetype case for a value renamed while crossing a seam; a swapped `primary_menu_active`/`notes_menu_active` pair was invisible to every existing test | `TransientDismissalEvidence` via `transient_dismissal_evidence()`; `test-utils`-gated, no automation projection | tier-2 | 7b | migrated |
| WFR-FOCUS-MODE | Focus Mode chrome suppression and readable-column margins | **4 files, 870 physical / 754 production** in `ui/window/focus_mode/**` (facade 338, policy 223 raw of which 116 are co-located tests, evidence 98, chrome 211). **Policy re-measured by `extend-kani-to-pure-policies` (design D10), same method (lines before the `#[cfg(test)]` module): 225 raw / 109 production before, 303 raw / 156 production after (+47, the relocated `readable_column_margin` and its two constants), 147 test lines**; still 4 files | `win.toggle-focus-mode`, `win.set-focus-mode`, the affordance, `Escape` after transient dismissal | `crates/lushtext-core/src/ui/window/focus_mode/policy.rs`. **Gained `readable_column_margin`** (design D10 of `extend-kani-to-pure-policies`): the readable-column margin was an unlisted whole-pixel decision in the called presentation surface `ui/editor_page/focus_mode.rs`, shared with the Markdown preview's `widgets.rs`; it now takes Pango units and is integer (`div_euclid` in `i64`), bit-exact with the `f64` form it replaced, whose reference stays in that surface's tests. The module forbids `clippy::float_arithmetic` and `clippy::disallowed_methods` | 0 inspection declarations remain; 0 actuation seams added | **done**: `FocusModeEntry` + `FocusModeRestore` — what the mode borrowed on entry and must give back on exit, which is what makes "restores only what it borrowed" assertable | `FocusModeEvidence` via `focus_mode_evidence()`; `test-utils`-gated, no automation projection | tier-2 | 7b | migrated |
| WFR-EDITOR-MEMORY-EVICTION | Bounded editor-memory eviction and its continuation | **4 files, 953 physical / 801 production** in `ui/window/editor_memory_eviction/**` (facade 333, policy 295 raw of which 152 are co-located tests leaving 143 production, evidence 161, execution 164), extracted from two discontiguous ranges of the 856-line retired ui/window/focus_indexing.rs (**407** production lines, re-derived; the inherited ~590 had absorbed the focus-restoration block between the two ranges) | tab selection, editor load and save completion, memory-policy change, the bounded idle continuation | `crates/lushtext-core/src/ui/window/editor_memory_eviction/policy.rs` | **8 before -> 2 after**, re-derived from the tree rather than carried: the retired ui/window/focus_indexing.rs declared 8 `*_for_test` functions for this story, and **6 of them were observation accessors** (`editor_memory_evaluation_count`, `_full_scan_count`, `_incremental_total`, `_reconciles`, `_outcome`, `_eviction_dispatch_count`) now retired into `EditorMemoryEvictionEvidence`. The **2 race-injector actuation hooks** (`set_before_editor_memory_eviction_hook_for_test`, `set_after_...`) are what actually crossed the move unchanged; **0 actuation seams added**. An earlier revision of this cell read *"8 test seams carried across the move unchanged"*, which was the pre-migration count restated as a post-migration one — the seam cell contradicted this row's own evidence surface, whose module doc says it replaced six single-counter accessors | **done**: the eviction generation identity, carried across the move without changing timing | `EditorMemoryEvictionEvidence` via `editor_memory_eviction_evidence()`; `test-utils`-gated | tier-3 | 7b | migrated |
| WFR-RECENT-DOCUMENTS | Reopen or forget a recently opened document | **7 files, 2,140 physical / 2,069 production**: canonical role home `ui/open_popover/**` (1,780 physical / **1,709** production across 5 files — facade 252, policy 153 raw of which 71 are co-located tests leaving 82 production, evidence 397, imp 892, item 86) plus the nested window half `ui/window/recent_documents_journal.rs` (215) and `ui/window/recent_open.rs` (145), neither of which carries a co-located test module. Components sum exactly: 252 + 82 + 397 + 892 + 86 + 215 + 145 = 2,069. Re-derived after the review pass under the same predicate the disposal row states; the earlier cell recorded `policy.rs` at 183 raw / 96 production, which no revision of the file has ever measured. The pre-7b attribution put all of this under `WFR-SHELL-LAYOUT`, which slot 3b assigned without re-deriving the row's cells | the header Open menu button, `win.open-recent`, `Ctrl+K`, `win.set-open-popover-query`, a recent row's activation or remove button, the chooser button | `crates/lushtext-core/src/ui/open_popover/policy.rs` — the two empty-state copies (an empty history and an over-filtered list look alike on screen and say different things), the announced list value text, and the skip-while-invisible rebuild rule | **26 gated declarations across 37 sites retired to 11 — and the 11 is the honest number, not a smaller one.** (This counts gated declarations at **any** visibility; the programme record's `*_for_test` ratchet counts `pub`/`pub(crate)` only and reads this row as 23 -> 11. Two predicates, two numbers, neither merged into the other.) 14 observation accessors folded into the evidence surface, the row's **ungated** `window.imp().recent_documents.loading` read in the widget test closed, and 2 imp-level inspection helpers removed. What remains is **11 test entry points, every one actuation or a widget handle, none an observation getter**: 2 setters (`set_search_text_for_test`, `set_list_scroll_value_for_test`), 5 widget handles for AT-SPI audits and event emission, `row_layout_snapshot_for_test` (a **class constant**, an associated function taking no instance — moved off the evidence surface because building it constructed eight GTK widgets per read), and 3 journal drives (`set_recent_documents_for_test` plus `remove_recent_document_for_test` and `merge_loaded_recent_documents_for_test`, the last two added so the reentrancy proof could drive **every** mutable-borrow path including the startup-merge branch a stale generation guards). **2 actuation seams added, counted and justified individually** rather than absorbed into a smaller headline | none required — the workflow's cross-boundary values are a `Vec<RecentDocumentRow>` one way and a `PathBuf` the other; neither is a bundle, and reifying either would invent a parallel shape | `OpenPopoverEvidence` via `open_popover_evidence()` and `RecentDocumentsJournalEvidence` via `recent_documents_journal_evidence()` — **two accessors because the workflow spans two GObjects** and a bare popover exists in tests with no window, the shape `WFR-WORKSPACE-TREE` established for a nested role home. `test-utils`-gated, no automation projection | tier-3 | 7b | migrated |
| WFR-SHELL-GEOMETRY | Adaptive shell geometry: the workspace and properties surfaces | **4 files, 1,703 physical / 1,176 production** in `ui/window/geometry/**` (facade 240, policy 828 raw of which 527 are co-located tests leaving 301 production, execution 490, evidence 145). Components sum exactly: 240 + 301 + 490 + 145 = 1,176. **Re-derived by `extend-kani-to-pure-policies`, which restructured the policy to whole pixels** (one tool, excluding `#[cfg(test)]` and `#[cfg(kani)]` items, reproducing 1,176 on the pre-change tree): **4 files, 1,789 physical / 1,236 production** — policy 840 / 347 (the `PaneShare` type and its constructors), execution 564 / 504 (`split_fraction`, the one fraction division, and its unit test), facade 240 and evidence 145 unchanged. The new `policy/kani_proofs.rs` is verification code and counted in neither figure. **Re-derived by `extend-closed-loop-geometry-verification`, which moved the allocation-time reconciliation decision into the policy** (same tool, excluding `#[cfg(test)]`, `#[cfg(any(test, kani))]`, and `#[cfg(kani)]` items, which reproduces 240 / 347 / 504 on the pre-change files): **4 files, 2,256 physical / 1,408 production** — policy 1,253 / 467 (`RenderedShellState`, `ShellWrite`, `ShellReconciliation`, `plan_shell_reconciliation`; the growth in physical lines is mostly the characterization tests and the exhaustive plan-versus-reference check), execution 614 / 554 (the plan's reader and applier, and the properties breakpoint's `apply`/`unapply` handlers that replaced its `layout-name` setter), facade 244 / 242 (the `kani_proofs` declaration and its table row), evidence 145 unchanged. The new `crates/lushtext-core/src/ui/window/geometry/kani_proofs.rs` (817 lines) is the breakpoint-loop model, verification code under `cfg(kani)`, counted in neither figure. Plus the two GTK adapter halves that stay behind by necessity: `ui/window/imp.rs`'s `size_allocate` vfunc and `constructed()` wiring, and `ui/window/actions.rs`'s two toggle action bodies | `win.toggle-sidebar`, `win.set-sidebar-visible`, `win.toggle-properties`, `win.set-properties-visible`, `F9`, window resize, breakpoint transitions, restore at construction | `crates/lushtext-core/src/ui/window/geometry/policy.rs` — relocated from the former ui/window/policy.rs with **81 -> 81 mutation parity measured from the tool on both sides**. Kani-proved by `extend-kani-to-pure-policies` in the harness module `policy/kani_proofs.rs` (verification code under `cfg(kani)`, outside the mutation scope and every size figure): layout exclusivity and requested-only rendering, the sheet/breakpoint agreement, breakpoint monotonicity, and positive pane shares, for every `i32` width, preset, and intent. **`extend-closed-loop-geometry-verification` added `plan_shell_reconciliation`** (with `RenderedShellState`, `ShellWrite`, `ShellReconciliation`): the writes `execution::sync_secondary_surfaces` and `sync_properties_breakpoint` used to decide inline — layout name, compact slot, both `show-sidebar` values, sheet `open`, threshold reinstall — as `Option` writes in the adapter's order, applied by execution, focus restoration staying there. Pure-policy consumers of the plan: **1 owning workflow** (this row: execution's two call sites) plus the breakpoint-loop model `crates/lushtext-core/src/ui/window/geometry/kani_proofs.rs`, which drives the real `derive_adaptive_shell_layout` and plan under a ledger-cited (A14–A18) Adwaita envelope and found two defects, both fixed failing-first: the properties breakpoint's `layout-name` setter flapped the layout inside one allocation under text scaling (the breakpoint now only triggers the reconciliation), and the Open-button breakpoint uncollapsed the workspace at text scales of 1.6 and up (it now sets `collapsed = true` too). Mutation, scoped to `policy.rs`: **80 mutants (66 before the plan, 14 in it), 69 caught, 11 unviable, 0 missed**. **The policy became whole-pixel in the same change** (maintainer decision): it returns whole-sp widths and `PaneShare { width_sp, of_sp }` values instead of `f64` fractions, the breakpoint is the exact integer ceiling, a rebased-properties floor that could never bind was dropped, and the one division into a split-view fraction moved into `execution.rs` as `split_fraction` (a coordination-side boundary conversion, unit-tested finite and in (0, 1]); `policy.rs` denies `clippy::float_arithmetic`. The row's cross-cutting `ui/sidebar/width_preset.rs` gained `width_preset/kani_proofs.rs`, one decision fix (a non-finite stored fraction resolves to `Comfy`, not `Large`, failing-first), and the same whole-pixel rewrite (`from_fraction` by comparison against the 0.25 and 0.35 midpoints, whole-sp `clamped_width_sp`, `effective_fraction` removed); the width-preset module stays outside this row's file set (it is a cross-cutting value), so the re-derivation in the size cell covers `ui/window/geometry/**` only | 1 inspection declaration retired (`workspace_sidebar_transition_pending_for_test`) into the evidence surface; 0 actuation seams added | **done**: `AdaptiveShellInputs` — constructed once per reconciliation and validated as a unit by the policy module | `ShellGeometryEvidence` via `shell_geometry_evidence()`. Its `persisted_workspace_fraction` / `persisted_properties_fraction` fields are the point: they make the allocation-path persistence rule **assertable** for the first time, where it was previously checkable only by reading the code or watching an animation stutter in the installed Flatpak | tier-3 — visual-sensitive, two named pixel invariants and the workspace-sidebar animation matrix | 7b | migrated |
| WFR-TAB-STRIP | Tab pin, reorder, and bulk close | **5 files, 1,046 physical / 958 production** in `ui/window/tab_strip/**` (facade 217, policy 223 raw of which 88 are co-located tests leaving 135 production, close\_execution 241, evidence 140, surfaces 225). Components sum exactly: 217 + 135 + 241 + 140 + 225 = 958, taking the whole of the retired ui/window/tabs.rs (634) plus the close/delete half of `ui/window/documents.rs` (`close_tab_for_path`). **The remainder of `documents.rs` is not this row's**: it is *called* by `WFR-DOCUMENT-SAVE` and `WFR-DOCUMENT-LOAD`, which both already record it as a neighbour they call and do not own | the tab context menu's five actions, `AdwTabView`'s close request and detach, sidebar delete and directory removal | `crates/lushtext-core/src/ui/window/tab_strip/policy.rs` — the pinned-segment invariant and every rule that follows from it | 0 inspection declarations before **and** after; the evidence surface is a **gain from zero**. 0 actuation seams added, so slot 5b's budgeted seam stays unspent | **done**: `TabLayoutEntry` — the value the GTK adapter and the pure policy both name, so every eligibility and move bound is decided over a `[TabLayoutEntry]` rather than over live pages | `TabStripEvidence` via `tab_strip_evidence()`; `test-utils`-gated, no automation projection | tier-3 | 7b | migrated |
| WFR-STARTUP-PREFLIGHT | App-data format preflight gating startup metadata consumers | **1 file, 519 physical / 490 production**: `ui/window/startup_data.rs`, up from **435 physical / 435 production** at `69f78b09` — the file had no co-located test module before this change. The growth is the bounded activation queue, its two pure predicates (`activation_queue_has_room`, `dropped_activation_message`), and the 29-line test module that covers them. Re-derived after the review pass, which replaced a first attempt that asserted the constant's own value. Attributed to `WFR-SHELL-LAYOUT` before slot 7b and owned by no row before that | app launch; a desktop or command-line activation arriving while the gate is closed | **none, and that is the conclusion rather than an omission.** The module was probed and **does** hold separable pure decisions — `startup_dialog_text`, `affected_data_summary`, `start_fresh_summary`, `metadata_title`, `format_item_count` — but their sole consumer is this gate, and the gate serves **five** other workflows, which is verbatim the cross-cutting criterion in `.agents/rules/workflow-convention.md`. They stay, and the probe's finding is recorded rather than a local `policy.rs` being manufactured | 0 gated declarations, 0 gate sites | none required — the gate's only crossing value is a `FormatPlan`, already a domain type | none required — a cross-cutting gate owes no surface; its observable state is the five workflows' own | tier-3 | 7b | cross-cutting — see [Cross-Cutting Coordination](#cross-cutting-coordination) |
| WFR-STATUS-NOTIFICATIONS | Status lane, inline alerts, notification lifecycle | **2,019 physical / 1,288 production** (ui 857 / services 431) across `ui/window/notifications/**`, `ui/status_bar/**`, `ui/info_bar/**`, and the shared `services/notifications.rs`. The pre-migration cell stated only physical and left the unit unlabelled; `services/notifications.rs` is 38% co-located tests | any workflow result, progress heartbeat, inline alert actions | **relocated to `ui/window/notifications/policy.rs`**: the pulse-visibility predicate, the sweep interval, and the inline-alert announcement key, text, and lane (moved out of the info-bar presentation surface, which should not decide what a screen reader hears). Gain from zero: **11 mutants** | **1 gated declaration / 1 gate site, re-derived as exact** — the census cell was right. **Retired to zero**: it was a test-only wrapper around a *pure* function, so its disposition was retirement onto production pure policy, not consolidation into a surface over nothing | **`EditorNotificationTarget`** — the census said `none`; the owner/surface pair was in fact reconstructed at four call sites from one editor owner id, which is the shape where an owner can be passed to a surface parameter unnoticed | **none, and that is the recorded conclusion**: after the seam retired the row has zero gated declarations, and every stage's result is observable through the bus's own production views | tier-1 | 7a | migrated |
| WFR-AUTOMATION-SPINE | Read-only D-Bus automation and action catalog | 5 files, 6,897 lines (ui 2,146 / model 2,195 / services 2,556) | D-Bus method calls, `scripts/lushtext-automation.py` | **none, and that is the conclusion rather than an omission** — re-derived in slot 7b by probing `ui/automation.rs` (2,214 physical / ~2,084 production) rather than by restating the cell. The probe **did** find separable pure decisions: the bounded-snapshot privacy boundary (`bounded_snapshot_text`, `bounded_len`, and their two constants), the exported contract vocabulary (`load_state_name`, `search_mode_name`, `local_history_availability_name`, `notification_severity_name`, `active_transient_surface_name`), the readiness helpers (`included_blocker`, `readiness_result`), and the GTK-coordinate conversions. They are **cross-cutting by the criterion in `.agents/rules/workflow-convention.md`**: their sole consumer is this module, and this module serves every workflow, so the rule's 'pure policy whose only consumer is its own coordination adapter is cross-cutting when that adapter serves several workflows' applies verbatim and they stay. `model/action_catalog.rs` remains domain with 3 consumers. **A finding recorded alongside:** `crates/lushtext-core/src/ui/automation.rs` is a literal path key in the native-minimap highlight and animation predicates in both visual-proof implementations, so turning it into a directory would disarm two named pixel invariants. That is a fact about the gate, not the reason for the verdict — the verdict is the rule's own criterion — but it is recorded so a later slot reaching for the extraction knows the re-key is part of the cost | **0 gated declarations, 2 `test-utils` gate sites**, re-derived in slot 7. The pre-slot-7 cell read `2 fns, 2 sites`: there are 2 gate sites in `ui/automation.rs`, but **neither gates a `fn *_for_test` declaration**, so the cell restated the gate-site count as a declaration count — the exact merge the unit discipline forbids, in the row whose subject is projection fidelity | none required — the exported contract is the value object | **7 projections registered** in `check-automation-docs.py`'s `EVIDENCE_PROJECTIONS`, re-derived from the tuple in slot 7b rather than inherited: `window.content_search`, `window.command_palette`, `window.tabs` from `SaveEvidence` **and** from `LoadEvidence`, `window.local_history`, `window.notes`, `window.workspace`. Slot 7a's correction to 7 is **confirmed**. Slot 7b registered **none**: all six of its new surfaces are `test-utils`-gated, so production cannot read them and there is nothing to project *from*. **But the Completion Rule still bit**, and the finding is recorded rather than waved: `window.surface`'s `workspace_sidebar_visible`, `workspace_sidebar_requested`, `document_properties_visible`, and `document_properties_requested` are `WFR-SHELL-GEOMETRY`'s facts, and `surface_snapshot` was **re-deriving** them — including re-implementing the sheet-versus-pane test by comparing `properties_layout_view.layout_name()` against the literal `"sheet"`. All four now read the workflow's own production accessors, which the evidence surface also derives from, so the exported contract and the surface share **one** derivation and are identical by construction. `MinimapEvidence`'s non-registration is **re-verified, not inherited**: the ≥18 `visual_geometry.native_minimap` fields derive from live widget geometry and adjustment state, not from workflow state, so there is nothing for a workflow surface to project — a result rather than an omission, now confirmed by a second slot | tier-1 | 2a onward, incrementally per migrated workflow; terminal status decided in 7b | cross-cutting — **terminal, decided in slot 7b**. The row is not `migrated` because it has no facade, no role home, and no ordered stage sequence of its own: it is a read-only projection that every workflow feeds, advanced incrementally by **eight** slots (2a, 2b, 3a, 3b, 4, 5a, 5b, 6) before slot 7b gave it its terminal status, so **nine** slots name it in the ledger. `exempt` was rejected in advance by the design, and rightly — a row eight slots kept advancing is not what `exempt` means. (An earlier revision of this cell said "nine" and "seven" in adjacent sentences; both are replaced by the ledger count.) The **ratchet row stays open and must not be struck retired**: the recorded occurrence at `ui/automation.rs:517`/`:518` is gone (`current_readiness_failure` now iterates `window.open_editors()`), but the *reading expression* persists at **8 sites** — `:585`, `:586`, `:776`, `:777`, `:836`, `:837`, `:843`, and the chained `.imp()` read at `:2004`, which a single-line grep misses. Three measurements, kept separate: the recorded occurrence (**retired**), the reading expression (**8 sites**), and the file's total `.imp()` reads (**15**). The eight sites read the tab collection, so their owning row is now [`WFR-TAB-STRIP`](#wfr-tab-strip) |
| WFR-EDITOR-MEMORY | Editor residency and memory policy | 1 file, 469 lines (model) | consumed by editor page load/save and window focus indexing | `model/editor_memory.rs` (7 consumer files across 5 modules and 3 workflows → cross-cutting). Kani-proved by `extend-kani-to-pure-policies` in `model/editor_memory/kani_proofs.rs`; to make it checkable the selection loop now takes the least-recently-used remaining page per step instead of sorting (same prefix, pinned by a reference unit test), and the ledger's exact accounting moved into a private `ResidencyTotals` value the ledger drives | n/a | exists: `EditorResidencyLedger` | none | tier-2 | none | exempt — see [Outlier Resolutions](#outlier-resolutions) — **probe evidence recorded in slot 7b (delta 1(b)'s retroactive re-check, task 1.4).** This row predates the probe rule and had none. Probed: `model/editor_memory.rs` is **entirely** pure decision logic already, so there is nothing to separate out; it has **7 consumer files across 5 workflows**, so relocating it beside any one of them would leave the other four reaching across a role home; and it is in the mutation scope through `model/**` already, so the convention would add no coverage. The `exempt` resolution is confirmed on that measurement rather than inherited. Note the row's **entry-point cell is now stale in its wording**: the retired ui/window/focus_indexing.rs is now `ui/window/palette_shell.rs`, and the editor-memory half of it is `WFR-EDITOR-MEMORY-EVICTION`'s role home |
| WFR-MIGRATION-LEDGER | Sidecar migration ledger | 2 files, 701 lines (model 225 / services 476) | note sidecar rename, startup reconcile | `model/migration_ledger.rs` (5 consumer files across notes, local history, and `services/` → cross-cutting) | n/a | exists: `MigrationLedgerEntry::matches_paths` | none | tier-3 | none | cross-cutting — **probe evidence recorded in slot 7b (task 1.4).** Probed: `model/migration_ledger.rs` is pure and `services/migration_ledger.rs` is its durable half; the pair has **5 consumer files across notes, format upgrade, and startup**, which is the cross-cutting criterion met on the measurement. No separable decision belongs to one workflow. Confirmed rather than inherited |
...
**One journal per application (`verify-multi-window-draft-journal`).** The lane
below is the application's, not the window's: `admission`'s `ProcessDraftJournal`
admits one registration, body-write pass, deletion, or orphan-cleanup pass at a
time across **every** window, and a window finding another window's work in it
marks itself pending. Its release resumes the waiting windows from **(11) one
cross-window idle wakeup** that calls each window's `drive_pending_draft_mutations`
while the lane stays free; a waiting cleanup resumes from its own follow-up timer.
The same coordinator claims each draft id for one window, lets only the first
window restore the session and schedule cleanup, and mirrors every commit into
every window's manifest copy.
...
**Added by `formal-verification-phase-0`.** Between candidate collection and the
first snapshot, both the autosave and close pipelines run
`register_new_draft_ids_then` ⇢ **(18) registration worker**: one batched
`update_manifest` registers every candidate id the persisted manifest lacks,
resuming in a completion that admits only candidates whose registration
committed (`journal_core::may_write_after_registration`, formerly
`policy::candidate_may_write_after_registration`). A stale draft
resolved by the lazy path goes through `retire_stale_draft` → the ordinary
delete queue, whose worker ⇢ **(19)** preserves the body first
(`draft_service::preserve_stale_draft_body`: local history, else
`drafts/set-aside/`) and deletes nothing when that fails, resuming in a
completion that publishes the destination-aware alert. The same change's
data-safety audit added the pending-restore autosave hold and
`preserve_unrestored_draft` ⇢ **(20) preservation worker**, resuming in a
completion that releases the hold. That makes **twenty** inversions.

**Changed by `consolidate-formal-verification-on-kani`.** No inversion was added
or removed; every decision the stages above take about a body — write, hold,
preserve, retire, delete, trust — now comes from the GTK-free journal core
`services/draft_service/journal_core.rs`, which a Kani harness checks for S1–S4
under crashes and restarts (`services/draft_service/kani_proofs.rs`). Every
unapplied-restore ending reaches `dispose_unapplied_restore`, which asks
`journal_core::unapplied_restore_disposition` whether to keep a copy; an
`Unavailable` restore now keeps one too (the harness's S1 counterexample). A body
write takes the `RegisteredDraft` token that only registration mints, and the
insert-only fallback commit runs through the one manifest command. Preserved
bodies have one owner, `services/draft_service/set_aside.rs`, which the
`Preferences > Data` page lists.

**Seventeen deferred inversions (before phase 0), where the census recorded seven worker
...
(that coordinator is now a type alias over the shared `SingleFlightCoordinator`;
see the seam entry below)
...
**Re-derived by slot 5a from the code: five stage orders and 28 resumption
points — 21 deferral primitives plus 7 non-primitive callback resumptions.** The
census cell recorded **four inversions**, so the floor was off by **7x**. The
five stage orders are editor note resolution (2 primitives), the bookmark
lifecycle (4), the note editors (9), the notes browser's source build plus query
(4 + the search debounce), and sidecar migration (2). Counted separately: 4
folder-chooser and dialog-response resumptions, 2 `connect_closed` teardowns, and
...
attributed "the startup format gate 4 plus a user-driven retry loop" to this row;
slot 5a decided `ui/window/startup_data.rs` is **cross-cutting, owned by neither
row** (it orders five workflows), so those resumption points are an ownership
correction removed from this trace rather than a count corrected within it.

The generation-plus-mode triple is reified: `NotesBrowserTicket` +
`NotesBrowserFacts` + one predicate, phantom-typed by flight because the three
stages own three different generation counters.
...
> **Slot 7 re-derived this row's inversion count and it is low by ~3.2x.** The
> recorded floor is **5**; counted by **actor**, the row owns **7 ordered stage
> orders and 16 resumption points** (14 excluding two passive signal re-entries).
> This is the sixth consecutive slot to find its inversion count low. The
> uncounted ones are: the settle→idle→50 ms-timeout code-block repair chain; two
> **passive re-entries by a different actor** (`notify::width` /
> `notify::left-margin` / `notify::right-margin`, and `connect_map`, both of which
> re-enter the repair machine with **no settle armed**); the 300 ms render
> debounce; the chunked source-capture completion; the superseding-plan
> re-dispatch inside its own completion closure; the serial image-queue drain
> re-entry; and — most significantly — **the retirement drain resuming a parked
> render or projection** (`mod.rs:1747`), a cross-actor backpressure inversion in
> which the *retirement* lane restarts *production* work, because
> `render_markdown_with_context` and `start_render_plan` both park on
> `markdown_retirement_at_capacity()` and only the drain un-parks them. A facade
> for this row must name that one explicitly.
...
**Re-derived from the code by slot 6: five stage orders and six resumption
points**, against the three inversions recorded here — a floor off by 2x, the
fifth consecutive slot to find its inversion count low.

| # | Stage order | Deferral primitive | Control resumes in |
| --- | --- | --- | --- |
| 1 | availability classification and analysis eligibility | none — synchronous | n/a |
| 2 | sliced content analysis | `glib::idle_add_local` | `analysis_execution::run_minimap_analysis_slice`, which re-checks generation **and** lifetime on entry, again after the slice, and once more before publishing |
| 3 | marker refresh | `MinimapState::refresh_debounce` (`Debounce`, `MINIMAP_REFRESH_DEBOUNCE` 80ms) | `projection_execution::refresh_minimap` |
| 4 | width-reflow freeze/settle/reveal | `MinimapState::reflow_settle` (`SettleBurst`, 150ms) | `reflow_execution::finish_minimap_reflow_settle` |
| 4 | " | that handle's `schedule_follow_up(MINIMAP_REFLOW_REVEAL_DELAY, ..)` (800ms) | the same module's quiet-window reveal |
| 4 | " | **out-of-band, different actor**: user scroll | `reveal_minimap_reflow_freeze_for_user_scroll`, which re-enters the live machine while the follow-up is still armed. `reflow_reveal_pending` exists precisely because the reveal and the burst are separately live |
| 4 | " | **passive re-entry**: `ViewportObserver` from `ui/editor_page/overscroll.rs` | `schedule_minimap_reflow_settle` — the same machine, entered **without** a freeze, which is the behavior contract `.agents/rules/ui.md` states |
| 5 | modified-line mark maintenance | none of its own; driven by four external workflows | load, save, local-history restore, and draft restore, each suspending edit tracking around its own buffer mutation |

The frame-timing-sensitive, pixel-verified inversion is stage 4's, and it is why
this row was deferred to slot 6 — for proof cost, not fit. **Resolved by slot 6.**
...
`WFR-PRINT`: `win.print` → read `PrintEvidence` → run (native operation or probe) → `policy` decides whether the outcome owes the user a message. No inversion: `PrintOperation::run` blocks on the native dialog and returns to the same call.
...
settled animation. This row **was** a residual grouping of 19 shell surfaces that
share the window adapter and have no coordination seam. Slot 7a established it
was never one workflow and slot 7b **superseded** it with seven rows, so this
entry is kept as the grouping's history rather than as a live claim: the sequence
above is `WFR-SHELL-GEOMETRY`'s, and that row's facade records **two**
inversions, so none of the seven successor rows belongs on this list.
...
| `exclude_re` entries | entries in `.cargo/mutants.toml` | **62** (50 services-scoped, 11 ui-scoped, 1 unscoped). Was 72 before slot 6, which retired **ten** minimap entries and re-keyed the four survivors |
| Minimap mutation exclusions | ui-scoped `exclude_re` entries naming minimap methods | **RETIRED by slot 6: 14 entries / 66 method names / 17 physical TOML lines → 4 entries / 0 method names / 4 physical lines.** Four adapter and GTK-wrapper entries retired because the adapter is out of scope for not being a policy module; four `line:column`-anchored entries **deleted for matching no generated mutant at all**; and of the 66 named methods, **seven had zero definitions anywhere in the tree**. The four survivors are all symbol-scoped equivalence claims re-verified against real mutants, and two of them only became narrow enough to state after the duplicated min-height expansion was extracted into one shared helper |
...
The programme's headline ratchet, now mechanical. `make check-workflow-boundaries`
recomputes the count and **fails when it exceeds** the figure below. A count
*below* the ceiling passes without a finding and does not have to be written down:
forcing the figure down on every cleanup would train a reader to treat it as a
number to adjust rather than a ceiling to stay under.
...
- externally reachable `\*_for_test` declaration ceiling: 165

Predicate, identical to the one
[`docs/next/workflow-readability.md`](next/workflow-readability.md) records this
figure against: `pub fn` and `pub(crate) fn` whose name ends `_for_test`, under
`crates/lushtext-core/src`, counted over comment- and string-stripped code so a
doc comment naming a retired getter cannot inflate it. A further 18 declarations
are `pub(super)` and are outside the predicate by design. Against the census's
**300**, and **188** at `69f78b09`.

Raising the ceiling is permitted and is a **reviewed decision**: do it in the same
change, with a stated reason, after establishing that the fact the test needs
genuinely cannot go on the owning workflow's evidence surface.

`cfg(feature = "test-utils")` attribute sites (**530**) are deliberately **not**
ratcheted. That population rises as workflows gain gated evidence surfaces, so
...
| ~~`minimap_analysis.rs`~~ | 186 raw / **120 production** | the pre-move `minimap.rs` **plus** `crates/lushtext-core/benches/benchmarks.rs` | **2 consumers, 1 owning workflow** (`WFR-MINIMAP`) | **DONE by slot 6** — relocated to `ui/editor_page/minimap/policy.rs` with mutant-by-mutant parity. The cell read `1 consumer`; the second is an external bench target, which is a consumer but not an owning workflow, and only the latter decides eligibility |
...
| `workspace_visibility.rs` (model) | 199 production | `services/filesystem/tree.rs`, `services/file_tree.rs`, `services/palette/index.rs`, `services/content_search/search.rs`, `ui/workspace_visibility.rs`, `ui/sidebar/workspace_section/scan_execution.rs`, `ui/sidebar/workspace_section/folder_execution.rs`, `ui/window/palette_shell.rs`, `ui/search_panel/imp.rs`, `ui/preferences/excluded_names.rs`, `ui/automation.rs` | 3 (`WFR-WORKSPACE-TREE`, `WFR-COMMAND-PALETTE`, `WFR-SEARCH-REPLACE`) | cross-cutting → stays in `model/`; mutation coverage is a **gain from zero** (new module) |
| `editor_memory.rs` | 469 | the retired ui/window/focus_indexing.rs (now `ui/window/palette_shell.rs`), `ui/window/imp.rs`, `ui/editor_page/mod.rs`, `ui/editor_page/save/admission.rs`, the retired ui/editor_page/load_runtime.rs | 3 | cross-cutting → stays, exempt |
| `migration_ledger.rs` | 225 | `ui/window/notes/journal.rs` (was `notes/mod.rs` before slot 5a), `ui/window/local_history/journal.rs`, `services/migration_ledger.rs` | 2 plus a service | cross-cutting → stays |
...
| The recent-documents **list surface**: popover row layout, row geometry, the list's own loading state (`recent_documents.loading`), and `OpenPopoverRowLayoutSnapshot` | **`WFR-RECENT-DOCUMENTS` (slot 7b)** — assigned to `WFR-SHELL-LAYOUT` by slot 7, and carried to that row's replacement when it was superseded. It is window-shell presentation, not load state: a row-layout snapshot answers "how is this row laid out", which no load stage asks |
...
This is recorded the way slot 2a recorded that the retired ui/window/focus_indexing.rs had
...
**The five notes domain modules are confirmed staying — decided by slot 5a**, and
the confirmation is by import rather than by substring. Each is consumed by at
least one **GTK-free service**, so relocating any of them under `ui/` would invert
dependency direction, which the convention forbids outright — the same ground on
which 2b settled `workspace_search.rs` and 3b settled `model/file_load.rs`:

| Module | GTK-free service consumer that forbids the move |
| --- | --- |
| `model/note.rs` | `services/note_storage.rs`, `services/palette/notes.rs` |
| `model/bookmark.rs` | `services/bookmark_service.rs`, `services/bookmark_excerpt.rs`, `services/palette/notes.rs` |
| `model/folder_note.rs` | `services/folder_note_service.rs` |
| `model/document_note.rs` | `services/document_note_service.rs`, `services/palette/notes.rs` |
| `model/sidecar_identity.rs` | **cross-workflow kernel** — consumed by note, bookmark, document-note, draft, local-history, search-backup, and format-upgrade code: **seven consuming workflows**, so it is not any one row's policy |

`WFR-NOTES-BOOKMARKS` therefore owns **no relocated policy at all**; its
`policy.rs` is a **gain from zero** extracted out of the GTK adapter. Substring
false positives named and excluded while deriving these sets: `note` matches
`connect_notify_local`, `notes_menu_button`, and `noted`; `bookmark` matches the
editor page's `connect_bookmarks_changed` and `bookmark_change_generation`. The
sets above were taken from `use` statements and `crate::model::` paths.

**`model/workspace.rs` is likewise confirmed staying** — it is consumed by the
workspace tree, notes, the palette, shell layout, search scope, and automation, so
it is domain with many owning workflows, not single-workflow policy.
...
`ui/window/notes/bookmark_execution.rs` (was `notes/mod.rs` before slot 5a) and
the retired ui/window/focus_indexing.rs;
...
### done: `NotesBrowserTicket` + `NotesBrowserFacts` (`WFR-NOTES-BOOKMARKS`)

Reified by slot 5a in `ui/window/notes/seams.rs`, closing the row's `required`
entry. The census cell described the bundle as `{generation, mode}` with the
`is_current(generation) && mode == mode && !disposed` triple duplicated at **two**
sites; the code has **three** completion sites, the third being the closed-file
bookmark-excerpt preview with a documented two-clause variant.

**The reification goes further than the cell asked, and the reason is the defect
class the seam rule exists for.** The three stages own **three different
generation counters** — the source refresh coordinator, the browser query
coordinator, and the excerpt preview coordinator. A plain `{generation, mode}`
struct would let a query generation be validated against source facts: both are
`u64`, both are called `generation`, and the mistake is invisible to review and to
tests because either value reads correctly on its own. The ticket is therefore
**phantom-typed by flight** (`SourceFlight`, `QueryFlight`, `PreviewFlight`), so a
cross-coordinator comparison is a **compile error** rather than a stale publish.

Mode gating is declared per flight through one associated constant.
`PreviewFlight` declares it `false`, which preserves the preview path's existing
weaker predicate **exactly**: a closed-file excerpt belongs to a selected *row*,
and its publication path revalidates that row's path and line through
`selected_bookmark_matches`, which is strictly stronger than a mode comparison.
Gating it on mode as well would have been a behavior change, so the weaker
predicate is declared rather than applied by accident — the shape slot 3b used for
`installation_is_current`.

Also closed here: **`NoteSourceRefreshCoordinator` is retired** into
`services::single_flight::SingleFlightCoordinator` as three type aliases, ending
the deferral slot 2a opened and slot 4 restated. The stated blocker — that
deduping the type changes `NotesBrowserRuntimeSnapshot`'s shape — dissolved
because slot 5a folds that snapshot into the row's evidence surface anyway. One
authoring claim was **stronger** than stated: the cancellation type did not merely
differ by name, because `NoteSourceRefreshStart` already carried
`PaletteSearchCancellation`, which `services/palette/runtime.rs:14` already
aliases to `single_flight::FlightCancellation`. ~100 duplicated lines of
submit/finish/supersede logic are gone, and the palette's instance changed at
...
### `minimap.rs` — RESOLVED by slot 6

**Migrated. This entry is kept as the record of a deferral that was honoured
rather than shortcut**; the row's current state is in
[Migrated Workflow Roles](#wfr-minimap-1).

The deferral was correct on its own terms and each of its three reasons was paid
rather than waived: the stream-frame visual geometry lane ran as a primary
acceptance gate, the 14 `exclude_re` entries retired only *after* the pure
projection math reached a `policy.rs` the mutation scope names by convention, and
the decomposition benefited from five settled facade shapes. Two of the three
premises were also found **wrong in the row's favour** when re-derived: the file
is 3,779 **raw** but 2,509 **production**, and the four `fit_projected_bounds`
exclusions among the fourteen had already stopped matching any generated mutant.
...
`minimap_analysis.rs`, so it *conforms in principle*: single-consumer
...
A later change MUST NOT promote it earlier to "get the big win first". Slot 6
did not: it was the slot the census assigned.
...
| 7a | Residual row sweep, **complete** | `WFR-PRINT`, `WFR-EDITOR-FIND`, `WFR-STATUS-NOTIFICATIONS`, `WFR-ENCODING`, `WFR-MARKDOWN-PREVIEW` migrated; `WFR-BUFFER-SNAPSHOT` surface obligations discharged; the last hand-listed `examine_globs` entry retired | tier-2 (preview) |
| 7b | Programme close (**complete**) | `WFR-SHELL-LAYOUT` superseded by seven rows, `WFR-PLAIN-DISPOSAL`'s lane surface, `WFR-AUTOMATION-SPINE`'s terminal status, capability deltas 1 and 2, the §E3 visual-proof re-key, the re-derived coverage proof, and the programme closeout. **The "remaining argument suppressions" item is discharged by recording rather than by removal**: exactly **one** `#[expect(clippy::too_many_arguments)]` exists, at `model/action_catalog.rs:178` — the exempt domain catalog constructor whose parameters each name a documented external contract field, which `.agents/rules/workflow-convention.md` places **outside** the seam rule — and **zero** `#[allow]` of that lint exist anywhere. Both counts re-derived in this change. The item is removed from this row rather than left to read as unfinished work | tier-3 (disposal) |
...
`ui/theme.rs`, `ui/accessibility.rs`, `ui/workspace_visibility.rs`, `ui/editor_page/accessibility.rs`,
...
**Populated in slot 7b (its task 5.5), per surface, with evidence for all three
properties the preamble claims.** The list's preamble asserts *no ordered
stages*, *no coordination role*, and *no seam value-object obligation*; before
this slot it asserted them for a bare name list, which is a claim without a
measurement.

| Surface | No ordered stages | No coordination role | No seam obligation |
| --- | --- | --- | --- |
| `ui/window/zoom.rs` (156 lines) | two `setup_*` functions that install controls and actions once at construction; every operation completes inside its own activation | nothing deferred, no worker, no generation, no timer | its only crossing value is an `i32` zoom step |
| `ui/window/workspace_scope.rs` (48 lines) | four accessors that read and write one scalar and push it to its consumers synchronously | same | one `WorkspaceScope` enum, already a domain value |
| `ui/properties_panel/**` (333 lines) | declares 2 `pub fn`s, has exactly one production caller (`documents.rs`, inside `refresh_status_bar`), and no shell stage order calls it | same | projection only |
| `ui/window/mod.rs`'s `setup_theme_selector` (~100 lines) | **slot 7b's task 0.5d decision.** A `gio::Settings` -> `libadwaita::StyleManager` projection installed once, plus a dark-notify handler. It is **not** a stage of the geometry sequence, and the check is concrete: it reads no paned position, no allocation, no breakpoint condition, and no width key, and nothing in the geometry sequence calls it | same | one `ColorScheme` value |

**Two entries the expectation named and that do NOT belong here**, recorded so
the correction is visible rather than read as an omission:

- `ui/open_popover/item.rs` — it is the row model object
  `WFR-RECENT-DOCUMENTS`'s factory binds, so it is that row's **called
  presentation surface**, not a surface outside the census. A migrated row's
  presentation surface is attributed *by the row*.
- the retired ui/window/transient_surfaces.rs — **must not be re-added**, and cannot be:
  it is retired. §D1 rejected it as a tier-list entry on the code, because it
  held a strictly ordered five-surface ladder plus a one-tick idle latch, which
  makes the "no ordered stages" claim false for it. It is now
  `WFR-TRANSIENT-DISMISSAL`.

**`ui/window/actions.rs` is not demoted here either.** It retains protected
geometry code — the two toggle action bodies that persist user intent — and is a
literal key in three visual-proof predicates. Demoting it would be a scope change
argued on a rename rather than on behaviour.
...
**Coverage proof — re-derived in slot 7b under a stated predicate.**

**Three figures were in circulation for one denominator** — this proof read
**198**, slot 7a recorded **266**, and a count taken while authoring slot 7b gave
**286** — and *that disagreement was itself the finding*. Its cause is now
stated, because a denominator without a predicate is not a measurement:

- the file count **grows with every migration**, because the convention replaces
  one large module with a facade plus several small role files. 198 -> 266 -> 286
  -> 306 is mostly the convention working, not drift;
- the three figures used **different predicates**. Whether separate-file
  `#[cfg(test)] mod tests;` modules count as production, and whether the crate's
  root-level infrastructure files are inside the denominator, each move the
  number by a handful. None of the three said which it used.

**The predicate, stated.** Every `*.rs` file under
`crates/lushtext-core/src`, minus files a sibling declares as
`#[cfg(test)] mod <name>;` (which a naive per-file scan counts as production),
minus the crate's root-level infrastructure files.

| Quantity | Count |
| --- | --- |
| `*.rs` files under `crates/lushtext-core/src` | **311** |
| separate-file `#[cfg(test)]` modules (`services/action_catalog/tests.rs`, `services/palette/tests.rs`) | 2 |
| production files | **309** |
| crate infrastructure at the crate root (`lib.rs`, `app.rs`, `config.rs`, `fuzzing.rs`) | 4 |
| **attributable to a matrix row or to the list above** | **305** |

The old proof's *"the remaining 3 are the crate infrastructure files named
above"* is corrected to **4**: `fuzzing.rs` was added after the census and the
proof was never re-derived. That is a one-file correction, and it is the kind the
re-derivation obligation exists to catch — a completeness claim that had been
true and quietly stopped being so.

**Attribution, by directory.** Every one of the 302 is reached:

| Files | Directory | Attributed to |
| --- | --- | --- |
| 24 | `model/**` | the domain rows and the workflows that own each value type; `model/editor_memory.rs` and `model/migration_ledger.rs` are the two `exempt` / `cross-cutting` rows, and `model/workspace_visibility.rs` is the cross-cutting entry-visibility rule shared by three migrated rows |
| 30 + 29 | `services/**` | the workflows that call them, per each row's "additionally calls" clause; `services/filesystem/**`, `services/json_store.rs`, `services/json_format.rs`, `services/single_flight.rs`, and `services/sync.rs` are in the list above |
| 7 | `ui/*.rs` | `accessibility.rs`, `theme.rs`, and `workspace_visibility.rs` (the GTK-side reader of the cross-cutting visibility rule) in the list above; `automation.rs` is `WFR-AUTOMATION-SPINE`; `buffer_snapshot.rs` is `WFR-BUFFER-SNAPSHOT`; `plain_disposal.rs` is `WFR-PLAIN-DISPOSAL`; `mod.rs` is the crate's `ui` module root |
| 10 | `ui/command_palette/**` | `WFR-COMMAND-PALETTE` |
| 13 | `ui/editor_page/*.rs` | `WFR-DOCUMENT-LOAD` and `WFR-DOCUMENT-SAVE` share the page; `accessibility.rs` is in the list above |
| 4 / 7 / 11 / 5 | the four `ui/editor_page/` role homes: buffer\_replacement, load, minimap, save | `WFR-BUFFER-REPLACEMENT`, `WFR-DOCUMENT-LOAD`, `WFR-MINIMAP`, `WFR-DOCUMENT-SAVE` |
| 2 | `ui/info_bar/**` | `WFR-STATUS-NOTIFICATIONS` |
| 17 | `ui/markdown_preview/**` | `WFR-MARKDOWN-PREVIEW` |
| 5 | `ui/open_popover/**` | `WFR-RECENT-DOCUMENTS` |
| 4 / 2 | `ui/preferences/**`, `ui/properties_panel/**` | the list above |
| 4 / 14 | `ui/search_bar/**`, `ui/search_panel/**` | `WFR-EDITOR-FIND`, `WFR-SEARCH-REPLACE` |
| 29 | `ui/sidebar/**` | `WFR-WORKSPACE-TREE`, except `file_tree_item.rs` (list above) and `width_preset.rs` (`WFR-SHELL-GEOMETRY`) |
| 2 | `ui/status_bar/**` | `WFR-STATUS-NOTIFICATIONS` |
| 15 | `ui/window/*.rs` | the shell rows; see the per-file attribution the seven replacement rows carry |
| 10 / 4 / 5 / 4 / 4 / 7 / 14 / 3 / 5 / 6 / 5 / 4 | the twelve `ui/window/` role homes: drafts, editor\_memory\_eviction, encoding, focus\_mode, geometry, local\_history, notes, notifications, print, session\_restore, tab\_strip, transient\_dismissal | the row of the same name |

**The split changed the attribution table**, which is why re-deriving it was
required work rather than a confirmation: six new role-home directories appear in
it that did not exist when the previous proof was written, and one directory
(`ui/window/`) lost three files to them while gaining two shared modules.
...
one line from its ceiling is not a margin, and **every later slot must plan against 1 line,
not 20.**
...
two stage orders will decide it. (That sentence was written when two workflows were migrated; twenty-two are now, and the observation held.) A
...
**Measured facades — all twenty-two, re-measured in slot 7b.** The number has
never been edited and no escalation past step one has been needed. Physical
lines. The verb is **re-measure**, not confirm: the previous table listed
**eleven** where **sixteen** rows were then `migrated`, omitting all five of slot
7a's, and two of the figures it did list were stale.
...
| `ui/window/focus_mode/mod.rs` | 338 | 32 |
| `ui/command_palette/mod.rs` (two stage orders, eight inversions) | 335 | 35 |
| `ui/window/editor_memory_eviction/mod.rs` | 333 | 37 |
| `ui/editor_page/load/mod.rs` (one stage order, **seven** inversions, seven entry points) | 317 | 53 |
| `ui/sidebar/mod.rs` | 293 | 77 |
| `ui/window/drafts/mod.rs` | 289 | 81 |
| `ui/open_popover/mod.rs` | 252 | 118 |
| `ui/markdown_preview/mod.rs` | 244 | 126 |
| `ui/window/geometry/mod.rs` | 240 | 130 |
| `ui/search_bar/mod.rs` | 229 | 141 |
| `ui/editor_page/save/mod.rs` (one stage order, five inversions) | 223 | 147 |
| `ui/window/tab_strip/mod.rs` | 217 | 153 |
| `ui/window/local_history/mod.rs` | 215 | 155 |
| `ui/window/transient_dismissal/mod.rs` | 199 | 171 |
| `ui/window/notes/mod.rs` | 178 | 192 |
| `ui/editor_page/buffer_replacement/mod.rs` | 169 | 201 |
| `ui/window/session_restore/mod.rs` | 165 | 205 |
| `ui/window/encoding/mod.rs` | 155 | 215 |
| `ui/window/notifications/mod.rs` | 153 | 217 |
| `ui/window/print/mod.rs` | 105 | 265 |

**Seven figures moved in slot 7b's re-measurement — four inherited and three of
this change's own.** The four inherited, each in a direction worth naming:
`ui/editor_page/load/mod.rs` **271 -> 277** (the table had it low by six),
`ui/search_bar/mod.rs` **238 -> 229**, `ui/markdown_preview/mod.rs`
**270 -> 244**, and `ui/sidebar/mod.rs` **292 -> 293**. The last two of those were
found while fixing a fail-open that had made the facade-budget rule silently
inert for three of sixteen migrated rows — which is what an unchecked rule
permits, and why a stale figure is the expected finding rather than a surprise.
**This change's own three were also stale by the end of the change**, and that is
the sharper lesson: they were measured before their module docs were finished,
which is the slot-5b failure mode repeating — geometry **164 -> 240**, tab strip
**223 -> 217**, recent documents **239 -> 252**. A facade figure taken mid-change
is a figure with a shelf life, and the table above is re-derived **after every
edit**, including the review fixes and the review's own follow-up pass.
`AGENTS.md`'s two module-layout figures for markdown preview and the search bar
were corrected to match in the same pass; a re-measurement that lands in one
document and not the other has simply moved the drift.

**`fix-markdown-preview-stale-after-load` moved two of these figures**:
`ui/editor_page/load/mod.rs` **277 -> 317**, because the content-republished
fan-out (`connect_content_republished` and its one-line firing wrapper) was
registered beside `connect_file_loaded` in the facade, the preview's
`content_is_installing` predicate joined `load_projection_suspended`, plus one
narration line in stage 7; and `ui/editor_page/buffer_replacement/mod.rs`
**168 -> 169**, one narration line in stage 7 naming the republish. Both were
re-measured after the last edit.

**Six facades were added by slot 7b** — the six replacement rows that carry one —
and every one landed under budget without escalation. The two tightest margins in
the repository are unchanged and both predate this change: the exemplar at **1**
and the minimap at **4**. Plan against 1.

**The previous version of this table was stale in both directions, which is why
task 9.3's verb is *re-measure* rather than *confirm*.** It was headed "after slot
3b", listed only four of the then-migrated facades, and recorded the load facade
at **253** where it now measures **271**. A budget claim checked against a stale
...
points than the save facade and still fits with 99 lines to spare, while the
exemplar's two stage orders sit 1 line under. Slot 6 (minimap) was expected to
prove the number wrong and did not: it landed at **366** after the programme's
first escalation step, a margin of **4**. Do not read 4 as headroom — the
repo-tightest margin is still **1**, and slot 2b's instruction to plan against 1
...
- called presentation surfaces: `ui/search_panel/imp.rs` (subclass state and template children), `ui/search_panel/list_factory.rs` (result-row projection), `ui/search_panel/item.rs` (the row model object the factory binds), `ui/search_panel/results.rs` (result-tree and revealer projection), `ui/search_panel/history.rs` (history and saved-search dropdown projection), `ui/search_panel/accessibility.rs` (accessible-state projection). Each is classified the same way in its own module doc. Named here as resolvable paths because the declaration is machine-checked; the bare-stem list under [Slot 5a amendment re-check](#slot-5a-amendment-re-check) stays as prose.
...
- `accessibility.rs` holds the accessible-state projection that used to sit in
  `mod.rs`. It is a called presentation surface, not a coordination role: the facade may not own
  widget mutation.
...
- facade: `crates/lushtext-core/src/ui/window/local_history/mod.rs` — **215** physical lines of the 370 budget (re-measured by slot 5a; the slot-4 figure was stale by one line)
...
- facade: `crates/lushtext-core/src/ui/window/drafts/mod.rs` — **317** physical lines of the 370 budget (re-measured by `consolidate-formal-verification-on-kani`, which added the `cleanup_journal` role-table line and declaration; `formal-verification-phase-0` had measured 312 after it added the write-ahead registration stage, the pending-restore hold, and three resumption points; slot 5a had measured 289, and the slot-4 figure of 310 was stale by twenty-one lines). It remains the programme's largest facade after the exemplar and the palette, but the exemplar's 369 is still the closest approach to the ceiling
- coordination: `crates/lushtext-core/src/ui/window/drafts/journal.rs`, `crates/lushtext-core/src/ui/window/drafts/cleanup_journal.rs`, `crates/lushtext-core/src/ui/window/drafts/admission.rs`, `crates/lushtext-core/src/ui/window/drafts/autosave_execution.rs`, `crates/lushtext-core/src/ui/window/drafts/restore_execution.rs`, `crates/lushtext-core/src/ui/window/drafts/retirement.rs`
...
- facade: `crates/lushtext-core/src/ui/editor_page/buffer_replacement/mod.rs` — **169** physical lines of the 370 budget (168 until `fix-markdown-preview-stale-after-load` added one stage-7 narration line naming the content republish), the programme's smallest facade so far (re-measured by slot 5a; the figure recorded at slot 4 was stale by one line)
...
- facade: `ui/window/notes/mod.rs` (**178** physical lines of the 370 budget)
- seam value objects: `ui/window/notes/seams.rs`
- coordination: `ui/window/notes/journal.rs` (the migration ledger), `ui/window/notes/source_execution.rs` (bounded note-source construction for both the browser and the command palette), `ui/window/notes/query_execution.rs` (the browser query), `ui/window/notes/bookmark_execution.rs` (bookmark lifecycle and editor note resolution), `ui/window/notes/editor_execution.rs` (document and folder note editors)
- policy: `ui/window/notes/policy.rs`
- evidence: `ui/window/notes/evidence.rs`
...
- facade: `crates/lushtext-core/src/ui/sidebar/mod.rs` — **293 physical lines of 370**
...
- called presentation surfaces: `crates/lushtext-core/src/ui/sidebar/callbacks.rs`, `crates/lushtext-core/src/ui/sidebar/dialogs.rs`, `crates/lushtext-core/src/ui/sidebar/imp.rs`, `crates/lushtext-core/src/ui/sidebar/workspace_section/mod.rs`, `crates/lushtext-core/src/ui/sidebar/workspace_section/imp.rs`, `crates/lushtext-core/src/ui/sidebar/workspace_section/row_factory.rs`, `crates/lushtext-core/src/ui/sidebar/workspace_section/context_menus.rs`, `crates/lushtext-core/src/ui/sidebar/workspace_section/row_accessibility.rs`, `crates/lushtext-core/src/ui/sidebar/workspace_section/icon_presentation.rs` — the same nine the prose below names, restated here as resolvable paths because the brace-expanded form is not machine-readable
- modules that are neither a role nor a called presentation surface: `crates/lushtext-core/src/ui/sidebar/workspace_section/watch_targets.rs` (a plain incremental data structure owned by the `watch` role) and `crates/lushtext-core/src/ui/sidebar/test_policy.rs` (the workflow's one test policy value), both explained in the prose below
- mutation parity: [`mutation-workspace-tree-policy.md`](../openspec/changes/archive/2026-08-28-migrate-workspace-tree-workflow-readability/evidence/mutation-workspace-tree-policy.md)
...
- `ui/sidebar/test_policy.rs` — the workflow's **one test policy value**, the
  `test-utils`-gated override store `.agents/rules/workflow-convention.md` requires a
  migrated workflow to own: "test-only timing and limit overrides live in the workflow's
  one `test_policy.rs`, and no override storage may compile without the test feature".
  It compiles to nothing without the feature and is not a second `policy.rs`.
...
owned by `WFR-SHELL-GEOMETRY` (it was `WFR-SHELL-LAYOUT`'s until that row was superseded) and consumed by Preferences and the window shell. It moved
...
2. five confirmed non-tree data-safety findings are handed on with owning rows, in
   [`data-safety.md`](../openspec/changes/archive/2026-08-28-migrate-workspace-tree-workflow-readability/evidence/data-safety.md);
...
4. the live `make run` walkthrough is deferred for user availability, per
   [`live-run.md`](../openspec/changes/archive/2026-08-28-migrate-workspace-tree-workflow-readability/evidence/live-run.md).
...
- called presentation surfaces: `ui/command_palette/imp.rs` (subclass state and template children), `ui/command_palette/item.rs` (the row model object the factory binds). Both carry the same classification in their module docs.
...
- `imp.rs` keeps the template children, the list factory, the accessible-state
  projection, and the source-installation helpers. It is a called presentation surface, not a
  coordination role.
...
**Role home: per-workflow subdirectory**, `crates/lushtext-core/src/ui/editor_page/minimap/`,
the fourth adopter in this directory after `save/`, `load/`, and
`buffer_replacement/`. Flat role names were mechanically unavailable:
`ui/editor_page/` hosts eight workflows and the fixed `policy.rs` / `evidence.rs`
names are already spent. A prefixed `minimap_policy.rs` was not a substitute — it
leaves the `ui/**/policy.rs` mutation scope, which the mutation-testing capability
classifies as a coverage regression that blocks the relocation.

- facade: `crates/lushtext-core/src/ui/editor_page/minimap/mod.rs` — **366 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/editor_page/minimap/policy.rs`
- evidence: `crates/lushtext-core/src/ui/editor_page/minimap/evidence.rs`
- coordination: `admission.rs` (availability classification, the O(1) wrapped-layout
  eligibility rule, generation/lifetime reservation), `analysis_execution.rs`
  (the bounded cursor and the sliced scan), `projection_execution.rs` (marker
  collection, strip drawing, native geometry sync, diagnostics),
  `reflow_execution.rs` (the freeze/settle/reveal state machine), `watch.rs`
  (buffer, adjustment, search, and settings observation), `retirement.rs`
  (cancellation, cursor and cache release, modified-mark clearing)
- test policy: `crates/lushtext-core/src/ui/editor_page/minimap/test_policy.rs`
- seam value objects: `MinimapProjectionSpace` and `MarkerProjectionSpace`, in
  `policy.rs` alongside the arithmetic that consumes them — the same placement
  `save/policy.rs` uses for `QueuedSaveTicket`. This row adds no `seams.rs`
- mutation parity: [`mutation-minimap-policy.md`](../openspec/changes/archive/2026-08-28-migrate-minimap-workflow-readability/evidence/mutation-minimap-policy.md)
  — relocation parity and extraction gain reported **separately**, each naming its
  invocation and file-level anchors, with the unfilterable 34-mutant floor stated
- data-safety fixes landed with this migration: [`data-safety.md`](../openspec/changes/archive/2026-08-28-migrate-minimap-workflow-readability/evidence/data-safety.md)

**Three stage-order-qualified `execution` modules, which is permitted rather than
a workaround.** Analysis, projection, and reflow are three distinct **ordered
stage orders** of one workflow, and the convention lets a workflow that owns
several qualify the bounded role name with the stage order it serves. The
alternative the convention explicitly forbids — taking an ill-fitting bounded
name for two of them — was the only other option, since `journal` does not apply
(the row keeps no durable record and reads nothing back at startup) and
`retirement` was already spent on the payloads that really are finished with.

**The freeze/settle/reveal machine is one module, deliberately.** It was tempting
to give the cover's removal to `retirement`, because it releases something. That
is rejected: the freeze, the settled repair, the quiet-window reveal, and the
early reveal on user scroll are **one state machine with a shared invariant** —
frozen pixels are revealed exactly once, after repair, and never while the burst
is pending. Splitting it would put
`reveal_minimap_reflow_freeze_for_user_scroll`'s
`reflow_settle.pending() || !reflow_reveal_pending.get()` guard on one side of a
role boundary and its setter on the other, which is the class of defect the seam
rules exist to make unrepresentable. `retirement` gets analysis payloads; it does
not get a live overlay.

**Two called presentation surfaces, which are not roles**, classified here and in
their own module docs:

- `crates/lushtext-core/src/ui/editor_page/minimap/widgets.rs` — the four
  read-only widget accessors `ui/automation.rs` reads from another workflow. It
  exists so those four reads are named operations rather than the `.imp()`
  reach-throughs they were.
- `crates/lushtext-core/src/ui/editor_page/imp.rs`'s **`MinimapState`**, which
  **stays where it is**. The taxonomy names exactly this shape — GTK subclass
  state and template children — and the classification has three supports:
  `dispose()` tears the row's state down in a documented order interleaved with
  the page's other subsystems, so moving the struct without moving `dispose()`
  would split a lifecycle; `minimap_overlay` is a `TemplateChild` bound by the
  Blueprint template and cannot leave `imp.rs` at all; and the migrated
  `WFR-DOCUMENT-SAVE` and `WFR-DOCUMENT-LOAD` rows already record
  `ui/editor_page/imp.rs` as the editor page's own shared state rather than
  theirs, so reversing that here would contradict two settled rows.

  The classification is recorded in **both** places the convention requires: the
  matrix row above, and `MinimapState`'s own doc comment in
  `ui/editor_page/imp.rs`, which names it a called presentation surface, states
  the three reasons it stays, and says it owns no `policy.rs` and no
  `evidence.rs`. Slot 5a's re-check found three of eight rows meeting only half
  of that requirement, and the first revision of *this* row met only half too —
  matrix plus facade, with nothing in the module's own doc.

**The facade budget held on its sixth test, at 366 of 370 with 4 lines of
margin — but only after one escalation step.** The projection was ≈300 with a
worst case near 340; the first honest facade measured **389**, over budget. The
declared escalation path's first response, *delegate harder*, was applied and
sufficed: the four widget accessors moved to `widgets.rs` as a called
presentation surface, which is where the taxonomy already said they belonged,
landing at 355. The cold read's seven fixes then took it to 362. The budget
number was **not** edited and the census row was **not** split. What
stressed the number here was neither stage orders (five) nor inversions (six) but
the **external entry surface**: 24 `pub`/`pub(crate)` operations called from 16
files outside the row, against load's seven — the programme's first data point
where entry-point count, not stage-order count, was the binding constraint.

**Two path-keyed gates were re-keyed by this migration, and the disarm was
observed before it was fixed.** Renaming the pre-move `minimap.rs` to
`ui/editor_page/minimap/` silently disarms both, greenly:

| Gate | Outcome | Evidence |
| --- | --- | --- |
| `.cargo/mutants.toml` `examine_globs` entry | **RETIRED**, not re-pointed. The whole purpose of extracting `policy.rs` is that the convention reaches the pure logic by name and the adapter is legitimately out of scope | Observed: after the move and before the config edit, `make mutants-list` generated **0** mutants for the old entry while still exiting 0, and 444 for `minimap/policy.rs` through `ui/**/policy.rs` |
| `native-minimap-highlight-anchors` / `native-minimap-animation-highlight-anchors` required-invariant predicate, implemented **twice** | **RE-KEYED** in both implementations, from `==` on one literal path to a prefix match on `crates/lushtext-core/src/ui/editor_page/minimap/`, so a later split inside the directory cannot disarm them again | Observed: the pre-fix predicate reported the new paths as still *visual-sensitive* but required **neither** invariant. Each implementation now carries its own parity assertion, and each was proved by a **deliberate red** — reverting the prefix to the old literal path fails `scripts/check-visual-proof-policy.py --self-test` and `cargo test -p cargo-gtk-proof minimap_role_home` respectively |

The Python half's self-tests were **unreachable** before this change: the module's
first `if __name__ == "__main__"` block delegated unconditionally to the Rust tool
and `sys.exit()`ed, so the `main()` and `run_self_tests()` below it never ran,
while `scripts/visual-geometry-smoke.py` imported the same module's predicates to
compute the recorded fingerprint. A parity assertion added there would have
asserted nothing. The CLI entry point now runs the Python self-tests before
delegating, and `make check-visual-proof-policy` invokes the script rather than
the Rust binary directly.

### WFR-PRINT

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/print/`.
`ui/window/` hosts many workflows, so the fixed `policy.rs` / `evidence.rs` names
cannot be shared at that directory's top level. (Until slot 7b the flat
ui/window/policy.rs name was taken by the adaptive shell geometry workflow;
that workflow took its own subdirectory in 7b, so the flat name is now free —
but every `ui/window/` workflow keeps a subdirectory, which is what makes the
choice legible rather than first-come.)

- facade: `crates/lushtext-core/src/ui/window/print/mod.rs` — **105 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/print/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/print/execution.rs` (`execution`) — owns the `PrintCompositor` and the `gtk4::PrintOperation` lifecycle, and is the one place GTK's result vocabulary becomes the workflow's
- evidence: `crates/lushtext-core/src/ui/window/print/evidence.rs` — `test-utils`-gated, so production never reads it
- test policy: `crates/lushtext-core/src/ui/window/print/test_policy.rs`
- seam value objects: none required — one synchronous request with no inversion, so there is no dispatch-time expectation to validate against a later completion
- mutation parity: **gain from zero**, `./scripts/run-mutants.sh list`, anchor `crates/lushtext-core/src/ui/window/print/policy.rs` — **0 mutants before, 3 after**. Not a parity claim: the module did not exist and its decisions were inline in the GTK adapter

**No inversion, stated because the absence is the interesting fact.**
`gtk4::PrintOperation::run` blocks on the native dialog and returns to the same
call, so the whole workflow completes inside the `win.print` activation. Nothing
resumes elsewhere and nothing needs a generation counter — the only request that
can be in flight is this one.

**`PrintDocumentSnapshot` folded in rather than kept.** It was a second typed
observation path over the same state. It is now `PrintEvidence`'s `document`
component, and the test probe receives the surface itself, so there is exactly one
observation path.

**The probe is retained, not retired.** `with_print_runner_for_test` is a
lifecycle probe: printing has no production stand-in, so there is nothing to
retire it onto. It is scoped to one call, so no override leaks between tests.

**Called presentation surface:** the status lane, reached through
`publish_status_message`. This workflow owns no widget.

### WFR-EDITOR-FIND

**Role home: flat** `crates/lushtext-core/src/ui/search_bar/`.

- facade: `crates/lushtext-core/src/ui/search_bar/mod.rs` — **229 physical lines of 370**, down from a 395-line pre-convention `mod.rs` (re-measured in slot 7b: the slot-6 figure of 238 was stale by nine lines, and the facade-budget rule had been silently inert for this row because the annotation's second backticked token defeated the claim parser)
- policy: `crates/lushtext-core/src/ui/search_bar/policy.rs`
- coordination: `crates/lushtext-core/src/ui/search_bar/execution.rs` (`execution`) — owns one search session: the `SearchContext` lifetime, navigation, replacement, and the scan projection
- evidence: **none, and that is the conclusion rather than an omission.** The row has **0** gated declarations and **0** gate sites, so there is nothing to consolidate; its observable state is already production API (`search_context`, `has_navigated`, `is_replace_revealed`, and the presentation surface's template children). Adding a surface would widen the API for no reader, which is the manufacture-a-role move the convention exists to stop
- seam value objects: none required — no identity or freshness bundle crosses two boundaries
- mutation parity: **gain from zero**, `./scripts/run-mutants.sh list`, anchor `crates/lushtext-core/src/ui/search_bar/policy.rs` — **0 mutants before, 36 after**

**The census's "fully synchronous ... no worker completion seam" is wrong.** The
row has exactly **one** inversion: `connect_occurrences_count_notify`.
GtkSourceView scans asynchronously, so the match total arrives from the scanner
rather than from the keystroke that caused it, and control resumes in
`execution::report_match_state`. The counter, the invalid-query styling, and the
throttled announcement are all projected there.

**Two called presentation surfaces, not roles:**
`crates/lushtext-core/src/ui/search_bar/imp.rs` (template children, button and key
wiring, revealer state, accessible labels) and
`cratesti/lushtext-core/src/ui/editor_page/search.rs` (the editor-side reveal,
focus, and cursor-restore choreography, which calls `attach` / `detach`). The
second lives in the editor page's widget tree rather than this directory, which is
why it is recorded here rather than named as a role.

**Entry Width Symmetry preserved verbatim.** The toggle-visible replace cells stay
wrapped in `GtkRevealer` with `row-spacing=0` and `margin-top` on revealed
children; nothing in this migration touched the grid, the revealers, or their
`slide-down`/150 ms transition.

### WFR-STATUS-NOTIFICATIONS

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/notifications/`,
resolved after §D1 because a flat ui/window/policy.rs was then claimed by the adaptive
shell geometry workflow. The window side is the canonical home because it is the
only layer that can see both surfaces at once.

- facade: `crates/lushtext-core/src/ui/window/notifications/mod.rs` — **153 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/notifications/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/notifications/execution.rs` (`execution`) — the bounded projection pass over the open tabs, and the recurring expiry sweep source
- evidence: **none, and that is the conclusion.** The row's single gated declaration **retired to zero** (below), leaving nothing to consolidate; every stage's result is observable through the bus's own production views (`status_bar_view`, `editor_info_bar_view`)
- seam value objects: `EditorNotificationTarget` in `policy.rs` — the bus owner and surface for one editor, derived once from its owner id instead of being rebuilt at four call sites
- mutation parity: **gain from zero**, `./scripts/run-mutants.sh list`, anchor `crates/lushtext-core/src/ui/window/notifications/policy.rs` — **0 mutants before, 11 after**

**The one seam retired rather than being consolidated.**
`inline_alert_announcement_key_for_test` was a `test-utils` wrapper around a
**pure function**, not an inspection of live state. Consolidating it into an
evidence surface would have built a surface over nothing; instead the function
moved into `policy.rs` as production API and the wrapper was deleted. The widget
test now reads the same function production reads. Seam count **1 → 0**.

**Two called presentation surfaces, not roles:** `ui/status_bar/**` projects one
status message onto the lane and owns the pulse animation; `ui/info_bar/**`
projects one notification onto an editor's inline alert row. Slot 4's honesty test
holds: **both import their view types from `services/notifications.rs` and define
no private copies**, and the info bar now imports its announcement decisions from
the canonical role home rather than deciding them itself.

**`services/notifications.rs` is shared, not this row's.** It is the GTK-free
reducer that decides which notification wins a surface, consumed by every workflow
that reports. It is named here with the rows that share it rather than pooled into
this row's size cell.

**The sweep is a heartbeat, not a settle timer.** The clock is a different actor,
so it is a real resumption point, but there is no newest request to supersede — it
must keep firing while notifications exist — so it stays an explicit
`timeout_add_local` per the settle-helper boundary in `.agents/rules/rust.md`.

### WFR-ENCODING

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/encoding/`,
for the same reason as the two rows above.

- facade: `crates/lushtext-core/src/ui/window/encoding/mod.rs` — **155 physical lines of 370**, down from a 907-line pre-convention `encoding.rs`
- policy: `crates/lushtext-core/src/ui/window/encoding/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/encoding/execution.rs` (`execution`) — the reopen hand-off, the lossy-encoding analysis and its confirmation, and the save-encoding, line-ending, and invisible-character writes
- evidence: **none, and that is the conclusion.** Both gated declarations are halves of **one timing override**, not inspections of live state, and every stage's result is read through the editor's own production accessors (`save_encoding`, `opened_encoding`, `save_line_ending`, `detected_line_ending`, `invisible_characters_mode`, `file_health`) plus the status lane
- test policy: `crates/lushtext-core/src/ui/window/encoding/test_policy.rs` — one `test-utils` configuration override, compiling to nothing without the feature
- seam value objects: none required. The freshness triple (analysis generation, content generation, still-active-tab) is validated by **one shared predicate**, `execution::analysis_is_current`, which both resumption points call — so the coordinator-generation form of the seam rule is satisfied without a new type
- mutation parity: **gain from zero**, `./scripts/run-mutants.sh list`, anchor `crates/lushtext-core/src/ui/window/encoding/policy.rs` — **0 mutants before, 32 after**

**Three stage orders, and the middle one has three resumption points.** Change save
encoding captures the buffer (chunked when large) → resumes in the capture
completion → analyses on a worker → resumes in the worker completion → presents a
lossy confirmation → resumes in its response handler. All three resumptions
recheck the same triple, **including inside the dialog handler**, because the user
can keep typing while it is open. Large-file mode skips the analysis and defers the
check to the save path rather than scanning the whole buffer on the GTK thread.

**One called presentation surface inside the role home:**
`crates/lushtext-core/src/ui/window/encoding/dialogs.rs` — the six grouped-row
dialogs and the shared row chrome. It is deliberately dumb about meaning: every
subtitle, description, and selected/activatable decision comes from `policy`, so
the Decision And Detail Dialogs wording contract is testable without constructing a
widget. **`ui/editor_page/invisibles.rs`** is the row's other called presentation
surface, applying a mode to one editor's `GtkSourceView` drawing flags; it lives in
the editor page's widget tree. It is the file the pre-migration size cell **omitted
entirely**.

**Dialog contracts preserved verbatim:** the grouped `AdwPreferencesGroup` sections
with one group per conceptual set, `title_lines(0)` / `subtitle_lines(0)` so long
copy wraps rather than truncating, activatable rows for choices and
non-activatable rows for facts, and the explicit inner content padding the Dialog
Text Surface Padding rule requires.

**`services/editor_io.rs` and `model/encoding.rs` are shared, not this row's.**
The first owns byte policy and is shared with the save and load rows; the second is
the domain vocabulary with 15 consumers and stays in `model/`. Neither was forked
to manufacture a local policy module.

### WFR-MARKDOWN-PREVIEW

**Role home: flat** `crates/lushtext-core/src/ui/markdown_preview/`.

- facade: `crates/lushtext-core/src/ui/markdown_preview/mod.rs` — **244 physical lines of 370**, down from a 1,983-line pre-convention `mod.rs` (re-measured in slot 7b: the recorded 270 was stale by twenty-six lines, and this row's facade-budget rule had been silently inert for the same parser reason)
- policy: `crates/lushtext-core/src/ui/markdown_preview/policy.rs` — inline-footnote lowering, the limited-plan shapes, and the fuzz/property entry points. Named `inline_footnotes.rs` until it took the role, which **retired the last hand-listed `examine_globs` entry** in the mutation configuration
- coordination: `admission.rs` (`admission` — ownership ceilings and the render session generation), `planning_execution.rs` and `projection_execution.rs` (**two `execution` stage orders, both stage-order-qualified** because one workflow owns two orders needing the same-shaped role), `retirement.rs` (`retirement` — bounded destruction and the backpressure inversion)
- evidence: `crates/lushtext-core/src/ui/markdown_preview/evidence.rs` — `test-utils`-gated, so production never reads it
- seam value objects: `crates/lushtext-core/src/ui/markdown_preview/seams.rs` — two families: the freshness/ownership seams (`PendingMarkdownRender`, `PendingMarkdownWork`, `GuardedMarkdownProjection`, `SnapshotMarkdownPlanOutcome`, `PlainRetirementTerminal`, `RetiredMarkdownRender`) and the render-time projection values that cross into the topical renderers and survive across turns (`EmbeddedBlockLayout`, `RenderedEmbed`, `ListFrame`, `ListMarker`, and the three render-state values)
- test policy: `crates/lushtext-core/src/ui/markdown_preview/test_policy.rs`
- called presentation surfaces: `crates/lushtext-core/src/ui/markdown_preview/imp.rs`, `crates/lushtext-core/src/ui/markdown_preview/widgets.rs`, `crates/lushtext-core/src/ui/markdown_preview/continuation.rs`, `crates/lushtext-core/src/ui/markdown_preview/text_flow.rs`, and the topical renderers `crates/lushtext-core/src/ui/markdown_preview/images.rs`, `crates/lushtext-core/src/ui/markdown_preview/tables.rs`, `crates/lushtext-core/src/ui/markdown_preview/code_blocks.rs`, `crates/lushtext-core/src/ui/markdown_preview/links.rs` — the topical decomposition two earlier changes paid for, deliberately not re-decomposed and carrying no role
- mutation parity: **relocation parity**, `./scripts/run-mutants.sh list`, anchor `crates/lushtext-core/src/ui/markdown_preview/policy.rs` — **175 mutants before the rename, 175 after**, configured total unchanged. A real parity claim rather than a gain from zero, because the hand-listed entry *did* select the file. All six of that file's path-prefixed `exclude_re` entries were re-keyed, and the unchanged count is what proves it: a broken exclude *raises* the count

**The recorded inversion count was low by ~3.2x.** The census recorded **5**;
this migration derived **7 ordered stage orders and 16 resumption points**,
counted by actor. The sixth consecutive slot to find its count low. Three of the
uncounted ones are why it matters:

- **the backpressure inversion**: when retirement is at capacity, the render and
  projection stages **park**, and only the retirement drain un-parks them. The
  *retirement* lane restarts *production* work — the reverse of the usual
  direction, and absent from every prior trace of this row;
- **two passive re-entries by a different actor with no settle armed** —
  `notify::width` / `notify::left-margin` / `notify::right-margin`, and
  `connect_map` — both re-entering the code-block width repair machine;
- **a second resumption inside one completion closure**, where a queued
  superseding plan is re-dispatched rather than starting a second worker.

**The row declared no evidence type of its own.** The pre-migration cell recorded
...
returned **bare tuples**, one of them a seven-element tuple whose reader had to
know that position 3 was a configured ceiling and position 4 a disposal job
count. Positional tuples are the specific defect the surface removes: a test
asserting `counters.3 == 2` reads as correct whichever field position 3 holds.
**Eleven of the thirteen retired** into `MarkdownPreviewEvidence` with five named
components; the two kept are actuators (`reset_image_work_observations_for_test`,
`reset_code_block_width_traversal_count_for_test`), which reset rather than
observe. **Test-only statics were 10, not the recorded 3**: three timing
overrides plus seven observation counters, and the seven counters became evidence
fields while the three overrides stayed test policy.

**Seven called presentation surfaces, which are not roles**, each recorded in its
own module doc: `widgets.rs` (the text view, scroller geometry, readable-column
margins, placeholder and failure states, embedded-widget insertion — following
the minimap's `widgets.rs` precedent), `imp.rs`, and the topical renderers
`code_blocks.rs`, `images.rs`, `links.rs`, `tables.rs`, `text_flow.rs`, and
`continuation.rs`.

**The preview non-goal was honoured: those topical modules were not
re-decomposed.** They carry the split two earlier changes paid for, and this
migration assigned roles to code that had **none** — the 1,983-line `mod.rs` —
while moving no responsibility between existing modules. The only edits the
topical modules received were **import paths**, because the seam values they
consume now live in `seams.rs`. One of those import rewrites orphaned a
`#[cfg(feature = "test-utils")]` gate in `images.rs`, which `--all-features`
could not see and the **default-feature rustdoc build caught** — recorded because
it is the same "a bulk substring rewrite is not a rename" hazard this slot hit
twice.

**`ui/window/preview.rs` is the window-side called presentation surface**, owning
the Alt+P shell workflow and the layout-settle path. It is **not** the only
external consumer: the note browser, bookmark, and editor surfaces are a
**second independent consumer** of the same widget, belonging to the migrated
`WFR-NOTES-BOOKMARKS` row. Measured entry surface: **48 externally-callable
operations, every one with a real caller**, across **11 external files**, while
all 63 `pub(super)` functions have **zero** external references — so the module
was already sealed and the facade needed no visibility widening.


### WFR-TRANSIENT-DISMISSAL

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/transient_dismissal/`.

- facade: `crates/lushtext-core/src/ui/window/transient_dismissal/mod.rs` — **199 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/transient_dismissal/policy.rs`
- coordination: none, and that is the conclusion rather than an omission. None of the five bounded role names describes a two-line idle latch, and manufacturing one to fill the slot is the move the convention exists to stop
- evidence: `crates/lushtext-core/src/ui/window/transient_dismissal/evidence.rs` — `test-utils`-gated, so production never reads it
- seam value objects: `TransientSurfaceState`, in `policy.rs`
- called presentation surfaces: `crates/lushtext-core/src/ui/window/transient_dismissal/surfaces.rs` — the workflow's only contact with widgets: live surface visibility, the palette hit test, and one `match` applying a dismissal verdict
- mutation parity: **gain from zero**, `MUTANTS_RE='transient_dismissal' ./scripts/run-mutants.sh full`, anchor `crates/lushtext-core/src/ui/window/transient_dismissal/policy.rs` — **0 mutants before, 10 after: 8 caught, 2 unviable, 0 survivors**. Not a parity claim: the retired ui/window/transient_surfaces.rs was not a `policy.rs` and no scope entry reached it

**Called presentation surface, carrying no role:** `surfaces.rs` — the surface
observation, the palette containment probe, and applying one dismissal verdict.
Each touches widgets and none owns an ordered stage.

**The one inversion** is a one-tick idle latch, named in the facade with its
mechanism, its resumption point, and the concrete bug it exists to prevent.

### WFR-FOCUS-MODE

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/focus_mode/`.

- facade: `crates/lushtext-core/src/ui/window/focus_mode/mod.rs` — **338 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/focus_mode/policy.rs`
- coordination: none — the mode's only deferred step is the affordance-hide `SupersedingTimer`, which is a settle helper the facade arms rather than a coordination job
- evidence: `crates/lushtext-core/src/ui/window/focus_mode/evidence.rs` — `test-utils`-gated
- seam value objects: `FocusModeEntry` + `FocusModeRestore`, in `policy.rs`
- called presentation surfaces: `crates/lushtext-core/src/ui/window/focus_mode/chrome.rs` — chrome hide/restore, the overlaid affordance and its accessibility state, and the per-editor presentation push; every decision it applies comes from `policy`
- mutation parity: **gain from zero**, anchor `crates/lushtext-core/src/ui/window/focus_mode/policy.rs` — **0 mutants before, 17 after: 16 caught, 1 unviable, 0 survivors**

**Called presentation surface, carrying no role:** `chrome.rs` — header-bar,
status-bar, and affordance visibility projection plus the per-editor readable
column application.

### WFR-EDITOR-MEMORY-EVICTION

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/editor_memory_eviction/`.

- facade: `crates/lushtext-core/src/ui/window/editor_memory_eviction/mod.rs` — **333 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/editor_memory_eviction/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/editor_memory_eviction/execution.rs` (`execution`) — the generation-guarded bounded idle continuation
- evidence: `crates/lushtext-core/src/ui/window/editor_memory_eviction/evidence.rs` — `test-utils`-gated
- seam value objects: the eviction generation identity, owned by the coordinator, which `.agents/rules/workflow-convention.md` names as itself the seam value object
- mutation parity: **gain from zero**, anchor `crates/lushtext-core/src/ui/window/editor_memory_eviction/policy.rs` — **0 mutants before, 22 after**

**`WFR-EDITOR-MEMORY` keeps its `exempt` resolution.** `model/editor_memory.rs`
is unchanged and stays in `model/`; this row owns the *window-side* eviction
orchestration only, and the design non-goal forbids widening the model row's
resolution to cover it.

### WFR-RECENT-DOCUMENTS

**Role home: nested.** The canonical role home is
`crates/lushtext-core/src/ui/open_popover/`, holding the facade, the single
`policy.rs`, and the single `evidence.rs`. The window half is nested in
`crates/lushtext-core/src/ui/window/`.

- facade: `crates/lushtext-core/src/ui/open_popover/mod.rs` — **252 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/open_popover/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/recent_documents_journal.rs` (`journal`) — startup recovery with pruning, the generation-guarded merge, and the debounced worker write. It is `journal` rather than `execution` because its whole contract is the record's lifecycle across process restarts
- evidence: `crates/lushtext-core/src/ui/open_popover/evidence.rs` — `test-utils`-gated
- seam value objects: none required, and that is the conclusion rather than an omission — the workflow's cross-boundary values are a `Vec<RecentDocumentRow>` one way and a `PathBuf` the other
- test policy: none required — the workflow has no test-only timing or limit override
- mutation parity: **gain from zero**, anchor `crates/lushtext-core/src/ui/open_popover/policy.rs` — **0 mutants before, 6 after: 5 caught, 1 unviable, 0 survivors**. Invocation `MUTANTS_RE='open_popover' MUTANTS_JOBS=1 MUTANTS_TEST_THREADS=4 ./scripts/run-mutants.sh full`, outcomes read from `mutants.out/outcomes.json` **filtered by file path** rather than from the run's headline counts, because `MUTANTS_RE` does not filter `delete field` mutants

**Called presentation surfaces, carrying no role and owning no policy or
evidence:** `ui/open_popover/imp.rs` (template binding, the row factory,
filtering, keynav), `ui/open_popover/item.rs` (the row model object the factory
binds), and `ui/window/recent_open.rs` (window-side wiring, action enablement,
and the row projection).

**Two accessors, deliberately.** The workflow spans two GObjects that are
separately observable — a bare `LushtextOpenPopover` exists in widget tests with
no window at all — so the surface exposes one accessor per owning object rather
than one that could only be called from half of them. This is the shape
`WFR-WORKSPACE-TREE` established for a nested role home, and each accessor still
reads its object's whole surface.

### WFR-SHELL-GEOMETRY

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/geometry/`.

- facade: `crates/lushtext-core/src/ui/window/geometry/mod.rs` — **244 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/geometry/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/geometry/execution.rs` (`execution`) — restore under a pre-clamp, breakpoint installation and re-tuning, and allocation-time reconciliation
- evidence: `crates/lushtext-core/src/ui/window/geometry/evidence.rs` — `test-utils`-gated
- seam value objects: `AdaptiveShellInputs`, in `policy.rs`; `RenderedShellState` and `ShellReconciliation` are the reconciliation plan's input and output, a seam only inside this row (policy to execution, and the Kani model)
- verification: `crates/lushtext-core/src/ui/window/geometry/kani_proofs.rs` — the breakpoint-loop model (`cfg(kani)`), declared in the facade; the policy's own harnesses stay in `crates/lushtext-core/src/ui/window/geometry/policy/kani_proofs.rs`
- mutation after `extend-closed-loop-geometry-verification`: **80 mutants, 69 caught, 11 unviable, 0 missed** (`cargo mutants --no-config -p lushtext-core -f crates/lushtext-core/src/ui/window/geometry/policy.rs`); 66 of them predate the plan (the whole-pixel rewrite had moved the file off the 81 below), 14 are the plan's
- mutation parity: **relocation parity, 81 mutants before and 81 after**, measured from the tool on both sides — `make mutants-list` at the old path from a clean worktree at `69f78b09`, and at `crates/lushtext-core/src/ui/window/geometry/policy.rs` in this tree. **Executed after the move: 77 caught, 4 unviable, 0 survivors**, path-filtered from `outcomes.json`. The file is byte-identical apart from visibility keywords, which cargo-mutants does not mutate. Slot 7a's separate **gain from zero (0 -> 80)** for the `adaptive_shell.rs` rename stands on its own; the 80 was stale by one and the tool now reports 81 at both paths
...
calls, which cannot move — and its `constructed()` wiring, delegating into
`execution.rs`; `ui/window/actions.rs` keeps the two toggle action bodies, which
persist user intent and announce it before handing the presentation decision to
the facade. Both files remain literal keys in the native-minimap highlight,
native-minimap animation, and workspace-sidebar animation-matrix visual-proof
predicates, and the role home was **added** to all three as a narrow prefix
rather than replacing them.

### WFR-TAB-STRIP

**Role home: per-workflow subdirectory** `crates/lushtext-core/src/ui/window/tab_strip/`.

- facade: `crates/lushtext-core/src/ui/window/tab_strip/mod.rs` — **217 physical lines of 370**
- policy: `crates/lushtext-core/src/ui/window/tab_strip/policy.rs`
- coordination: `crates/lushtext-core/src/ui/window/tab_strip/close_execution.rs` (`execution`, stage-order-qualified) — the close stage order and its cancellable-detach inversion. Qualified because this workflow owns several ordered sequences and only this one needs a coordination module
- evidence: `crates/lushtext-core/src/ui/window/tab_strip/evidence.rs` — `test-utils`-gated
- seam value objects: `TabLayoutEntry`, in `policy.rs` — the value the GTK adapter and the pure policy both name
- called presentation surfaces: `crates/lushtext-core/src/ui/window/tab_strip/surfaces.rs` — live `AdwTabView` page order, the context-menu model, the five menu-backed actions' enabled state, and the pin indicator
- mutation parity: **gain from zero**, anchor `crates/lushtext-core/src/ui/window/tab_strip/policy.rs` — **0 mutants before, 46 after: 46 caught, 0 unviable, 0 survivors**, path-filtered from `outcomes.json`

**Called presentation surface, carrying no role:** `surfaces.rs` — page-order
snapshots, the context-menu model, action enablement, and the pin indicator.

**Slot 7a's teardown-before-close fix crossed the move without being
duplicated**, and that was proved rather than asserted: the pre-fix shape was
deliberately reinstated and
`sidebar::test_close_tab_for_path_defers_teardown_until_the_page_detaches`
failed on it, then passed again once restored. The teardown exists **once**, in
the detach terminal.
...
- Every `.rs` file in the row's role home is the facade, `imp.rs`, a fixed role
  name, a bounded or stage-qualified coordination role, or is named by a
  **backticked repository path** in this row's own matrix text. The declaration
  must be resolvable: a bare stem or a brace expansion classifies a module for a
  human reader while leaving the check blind, which is how nine correctly
  classified modules read as undeclared until
  `harden-workflow-convention-guidance` restated them as paths.
- The externally reachable `*_for_test` count stays within the ceiling recorded
  under [Externally reachable test-seam ceiling](#externally-reachable-test-seam-ceiling).
  Raising it is permitted, in the same change, with a stated reason.
...
| `WFR-COMMAND-PALETTE` | **partial gap.** The size cell re-derived the `ui` subtotal and named one census misattribution (the retired ui/window/focus_indexing.rs, 856, which "stays window code"), but kept `services 7,897` pooled with no sharing rows named | size cell completed below |
...
reads that cross a workflow boundary; slot 5b retired **two** and slot 6 retired
**four** plus a fifth it discovered, so **two** remain — both `WFR-SHELL-LAYOUT`'s.
...
| ~~`:1152`, `:1159`, `:1177`, `:1239`~~ | ~~`editor.imp().scrolled_window`, `editor.imp().minimap_overlay`, `editor.imp().minimap.source_map`, `editor.imp().minimap.marker_strip`~~ | **RETIRED by slot 6.** All four now read named operations: `editor_viewport_widget()` on the editor page, and `minimap_shell_widget()` / `minimap_source_map_widget()` / `minimap_marker_strip_widget()` from `ui/editor_page/minimap/widgets.rs`, a recorded called presentation surface. A **fifth** minimap reach-through of the same class was found during the retirement and was not in this table — `editor.imp().minimap.render_hold` for the freeze cover — and it retired with them through `minimap_reflow_freeze_cover()`. `ui/automation.rs` now contains **zero** `editor.imp()` reads. The widget harness's **four** copy-pasted `minimap_source_map`/`minimap_marker_strip` helpers (two each in `tests/widget/editor_page.rs` and `tests/widget/window.rs`) retired onto the same accessors, widened to `pub` under `#[cfg(feature = "test-utils")]` only, so `tests/widget/**` also holds zero `.imp().minimap` reads |
...
Slot 5a amends two capabilities with **four** statements, so under section 8 every
already-migrated row is re-checked against all four. Three were *expected* to be
confirmations. **One was not, and it was not**: statement (b) found **three of eight
rows** owning a module that is none of the five roles and recording it under an
undefined label rather than as a called presentation surface — `WFR-SEARCH-REPLACE`
(six such modules), `WFR-COMMAND-PALETTE` (two), and `WFR-LOCAL-HISTORY` (one,
already named in this matrix but unclassified in its own module doc). Nine modules
across three rows. Slot 3b's re-check
found one of three rows non-compliant and slot 4's found two of four; this is the
widest gap the obligation has surfaced, and it is filled here.

The four statements:

- **(a)** a migrating workflow *classifies* its pre-convention focused siblings
  rather than choosing between the topical requirement and the role requirement;
- **(b)** a called presentation surface is **not a role**, is outside the
  five-name taxonomy, owns no `policy.rs` and no `evidence.rs`, and is recorded in
  its own module doc **and** named in its matrix row;
- **(c)** the two permitted role homes MAY be **nested**;
- **evidence surfaces** MUST NOT materialize toolkit state or advance a metric
  they report, and a field aggregated over a variable-sized child collection MUST
  be bounded, honest at zero, and skip a disposed child.

| Row | (a) topic name without a role | (b) module that is none of the five | (c) role home | evidence reads a lazy collection or its own counter |
| --- | --- | --- | --- | --- |
| `WFR-SEARCH-REPLACE` | none — not in a directory governed by (a) | **yes, six**: `imp.rs`, `list_factory.rs`, `item.rs`, `results.rs`, `history.rs`, `accessibility.rs`. Recorded in the facade's role table as "adapter detail", which is **not** a defined category. **Filled here**: named below and classified in each module doc | flat | no |
| `WFR-COMMAND-PALETTE` | none | **yes, two**: `imp.rs`, `item.rs`, likewise recorded as "adapter detail". **Filled here** | flat | no — `evidence()` reads a `gio::ListStore` length, which is fully materialized |
| `WFR-DOCUMENT-SAVE` | none | **no** — the row's five modules are all roles; `ui/editor_page/imp.rs` is the editor page's own shared state, not this row's | per-workflow subdirectory | no |
| `WFR-DOCUMENT-LOAD` | none | **no** — seven modules, all roles | per-workflow subdirectory | no |
| `WFR-BUFFER-REPLACEMENT` | none | **no** — four modules, all roles | per-workflow subdirectory | no |
| `WFR-SESSION-RESTORE` | none | **no** — six modules, all roles | per-workflow subdirectory | no |
| `WFR-LOCAL-HISTORY` | none | **yes, one**: `ui/editor_page/local_history.rs`. Slot 4 **did** name it in this matrix as a "called surface", so the matrix half held — but its **module doc** stated the ownership *rationale* without classifying the module, so the two-place requirement was half met. **Filled here** | nested-adjacent: canonical role home plus a called surface in a sibling directory | no |
| `WFR-DRAFT-RECOVERY` | none | **no** — ten modules, all roles plus `seams.rs` and `test_policy.rs` | per-workflow subdirectory | no |

**Statement (a) is a confirmation across all eight**, as expected: the two topical
requirements govern `ui/window/notes` and `ui/sidebar/workspace_section`, and no
migrated row lives in either. Slot 5a is the first row (a) applies to, and it
classified all four of its focused siblings.

**Statement (c) is a confirmation across all eight**: no migrated row has a nested
role home. `WFR-LOCAL-HISTORY` spans two *sibling* directories, which slot 4
resolved with canonical-home-plus-called-surface — the resolution (c) generalizes
to the nested case, not an instance of it. `WFR-WORKSPACE-TREE` will be (c)'s first
adopter when slot 5b migrates it; the analysis is already recorded in
`evidence/shared-ownership-decisions.md` §2.5.

**The evidence-surface statement is a confirmation across all eight**, and the
reason is worth stating rather than asserting: the hazard needs a **lazily created**
toolkit collection, and only `GtkTreeListModel` creates children on demand.
`WFR-COMMAND-PALETTE` and `WFR-SEARCH-REPLACE` read `gio::ListStore` lengths,
which are fully materialized; the four editor-page and window rows read scalars,
`RefCell` counts, and one manifest **count that is never cloned**. `WFR-WORKSPACE-TREE`
is the row the statement exists for, and slot 5a's own row proved the sibling
disposal half of it by **panicking**: the notes surface's first draft called
`active_editor()`, whose `TemplateChild<AdwTabView>` accessor panics after
`dispose()`, and the mandatory disposal proof caught it before it shipped.

#### The gap (b) found, and how it is filled

Three rows recorded a total of nine widget-projection modules as **"adapter
detail"**. That label
appears in two facade role tables and in this matrix, and it is defined **nowhere**
— not in `gtk-adapter-module-boundaries`, not in
`workflow-readability-boundaries`, not in `.agents/rules/`. It was doing exactly
the job statement (b) now names, under a name no gate and no reader could check.

Filled in this change, in both required places:

| Row | Called presentation surfaces, now named |
| --- | --- |
| `WFR-SEARCH-REPLACE` | `ui/search_panel/imp.rs` (subclass state and template children), `list_factory.rs` (result-row projection), `item.rs` (row model object), `results.rs` (result tree projection), `history.rs` (history popover projection), `accessibility.rs` (accessible-state projection) |
| `WFR-COMMAND-PALETTE` | `ui/command_palette/imp.rs` (subclass state, template children, list factory, accessible-state projection), `item.rs` (row model object) |
| `WFR-LOCAL-HISTORY` | `ui/editor_page/local_history.rs` (per-tab capture surface) — matrix half already held; the module doc now classifies it |
| `WFR-NOTES-BOOKMARKS` | `ui/window/notes/browser.rs`, `menu.rs`, `chrome.rs`, `targets.rs` |

Each of those modules' own doc now states that it is a called presentation
surface of its workflow, carries no role, and owns no `policy.rs` or
`evidence.rs`. **"Adapter detail" is retired as a label**: the two facade role
tables that used it now say "called presentation surface (no role)".

**None of the settled conventions is otherwise touched.** Re-confirmed
individually: role file names (`policy.rs`, `evidence.rs`, one of each per
workflow) unchanged; the bounded coordination role set
(`admission`, `execution`, `retirement`, `watch`, `journal`) unchanged and **not**
widened — statement (b) states the taxonomy's *scope* rather than adding a sixth
name; the facade budget line unchanged at 370 and **not edited**, with slot 5a's
facade measuring 178; the seam value-object Ticket/Facts/predicate shape unchanged;
...
[`retroactive-recheck.md`](../openspec/changes/archive/2026-08-28-migrate-workspace-tree-workflow-readability/evidence/retroactive-recheck.md).
