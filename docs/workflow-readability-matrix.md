...
| WFR-PLAIN-DISPOSAL | Off-GTK retirement of large owned payloads | 2 files, 2,227 lines (ui 1,535 / model 692) | called by 21 files across 10 workflows | `model/plain_disposal.rs` (1 consumer file, but its consumer is this module's own adapter → cross-cutting, stays) | 4/1/1/2 = 8 gated declarations. **Gate sites need their predicate, not a single number**: `ui/plain_disposal.rs` carries **17** `cfg(feature = "test-utils")` sites **plus 13** `cfg(any(test, feature = "test-utils"))` sites; `model/plain_disposal.rs` carries **0** of either. The pre-slot-7 cell read `18 sites`, which matches **neither** predicate and no arithmetic over them. This is the only row in the census using the dual gate, and the 13 dual-gated sites are where the `DisposalProducer` family lives — which is why that family reports `never used` under default features and is invisible to `clippy --all-features` | exists: `DisposalOwned<T>` + `DisposalPermit` | exists: `DisposalPressureEvidence` | tier-3 | 7b | cross-cutting — see [Cross-Cutting Coordination](#cross-cutting-coordination) |
...
| WFR-AUTOMATION-SPINE | Read-only D-Bus automation and action catalog | 5 files, 6,897 lines (ui 2,146 / model 2,195 / services 2,556) | D-Bus method calls, `scripts/lushtext-automation.py` | `model/action_catalog.rs` (3 consumers → domain, stays) | **0 gated declarations, 2 `test-utils` gate sites**, re-derived in slot 7. The pre-slot-7 cell read `2 fns, 2 sites`: there are 2 gate sites in `ui/automation.rs`, but **neither gates a `fn *_for_test` declaration**, so the cell restated the gate-site count as a declaration count — the exact merge the unit discipline forbids, in the row whose subject is projection fidelity | none required — the exported contract is the value object | exists: 18 `Automation*Snapshot` types; these become projections of workflow evidence as each workflow migrates. **Seven projections are registered** in `check-automation-docs.py`'s `EVIDENCE_PROJECTIONS` as of slot 7 — `window.content_search`, `window.command_palette`, `window.tabs` from `SaveEvidence` **and** from `LoadEvidence`, `window.local_history`, `window.notes`, and `window.workspace`. The four named below were the count as at slot 3b, and this cell carried that stale figure through four further slots; a row about projections carrying a stale projection count is the drift this census exists to close. The original four: `window.content_search` from `SearchPanelEvidence` (slot 1); `window.command_palette` plus both palette readiness blockers from `CommandPaletteEvidence` (slot 2a); `tabs[].saving` from `SaveEvidence` (slot 3a); and `tabs[].load_state` from `LoadEvidence` (slot 3b). `make check-automation-docs` gates all four against the `Evidence Projection Map` in `docs/automation-reference.md`. Since slot 3b, `tabs` is fed by **two** evidence surfaces at once, so the gate attributes a projected field by the local binding it is read through and keys the documented map by evidence type; without that, each surface would appear to project the other's fields | tier-1 | 2a onward, incrementally per migrated workflow; terminal status decided in 7b | pending |
...
| 7b | Residual close-out, **outstanding** | `WFR-PLAIN-DISPOSAL`, the `WFR-SHELL-LAYOUT` hybrid §D1 selected, `WFR-AUTOMATION-SPINE`'s terminal status, capability deltas 1 and 2, remaining argument suppressions, matrix completion | tier-3 (disposal) |
...
**Coverage proof — STALE, and by a wide margin. Re-derivation is required work
for the change that closes the programme.** This proof reads: *198 files exist
under `crates/lushtext-core/src`; 195 are attributed to a matrix row or to the
list above; the remaining 3 are the crate infrastructure files named above.*
Layer totals at census time: `ui/` 67,200 lines across 104 files, `model/`
11,817 across 29 files, `services/` 47,273 across 61 files.

Measured in slot 7: **266 `.rs` files** exist under `crates/lushtext-core/src`.
The proof is understated by **68 files (34%)**, so "195 attributed" cannot be a
completeness claim about the current tree. A programme that claimed completeness
while its own coverage proof described a tree 68 files smaller would be making a
false claim at exactly the moment it matters most. The re-derivation must also
absorb three attribution corrections slot 7 established:
`ui/window/search.rs` (955 physical) belongs to
[WFR-SEARCH-REPLACE](#wfr-search-replace) and was attributed to nothing;
`ui/properties_panel/**` (333) belongs to the
[no-coordination-tier list](#surfaces-with-no-coordination-tier) and **not** to
`WFR-SHELL-LAYOUT`'s file set; and `ui/window/focus_indexing.rs`'s
editor-memory eviction half is attributed to `WFR-SHELL-LAYOUT` while being owned
by no story anywhere.
...
**Measured facades — all eleven, re-measured in slot 7.** The number has never
been edited and no escalation past step one has been needed. Physical lines.
...
| `ui/editor_page/load/mod.rs` (one stage order, **seven** inversions, seven entry points) | 271 | 99 |
...
The flat `ui/window/policy.rs` name is taken by the adaptive shell geometry
workflow, so the fixed `policy.rs` / `evidence.rs` names cannot be shared at that
...
- facade: `crates/lushtext-core/src/ui/search_bar/mod.rs` — **238 physical lines of 370**, down from a 395-line pre-convention `mod.rs`
...
resolved after §D1 because a flat `ui/window/policy.rs` is claimed by the adaptive
...
- facade: `crates/lushtext-core/src/ui/markdown_preview/mod.rs` — **270 physical lines of 370**, down from a 1,983-line pre-convention `mod.rs`
