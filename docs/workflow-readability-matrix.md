...
committed (`policy::candidate_may_write_after_registration`). A stale draft
...
- facade: `crates/lushtext-core/src/ui/window/drafts/mod.rs` — **312** physical lines of the 370 budget (re-measured by `formal-verification-phase-0`, which added the write-ahead registration stage, the pending-restore hold, and three resumption points; slot 5a had measured 289, and the slot-4 figure of 310 was stale by twenty-one lines). It remains the programme's largest facade after the exemplar and the palette, but the exemplar's 369 is still the closest approach to the ceiling
