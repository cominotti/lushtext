...
| WFR-SHELL-GEOMETRY | Adaptive shell geometry: the workspace and properties surfaces | **4 files, 1,703 physical / 1,176 production** in `ui/window/geometry/**` (facade 240, policy 828 raw of which 527 are co-located tests leaving 301 production, execution 490, evidence 145). Components sum exactly: 240 + 301 + 490 + 145 = 1,176, plus the two GTK adapter halves that stay behind by necessity: `ui/window/imp.rs`'s `size_allocate` vfunc and `constructed()` wiring, and `ui/window/actions.rs`'s two toggle action bodies | `win.toggle-sidebar`, `win.set-sidebar-visible`, `win.toggle-properties`, `win.set-properties-visible`, `F9`, window resize, breakpoint transitions, restore at construction | `crates/lushtext-core/src/ui/window/geometry/policy.rs` — relocated from the former ui/window/policy.rs with **81 -> 81 mutation parity measured from the tool on both sides**. Kani-proved by `extend-kani-to-pure-policies` in the harness module `policy/kani_proofs.rs` (verification code under `cfg(kani)`, outside the mutation scope and every size figure): layout exclusivity and requested-only rendering, the sheet/breakpoint agreement, breakpoint monotonicity, and finite fractions in (0, 1], for every `i32` width, preset, and intent. The row's cross-cutting `ui/sidebar/width_preset.rs` gained `width_preset/kani_proofs.rs` and one decision fix (a non-finite stored fraction resolves to `Comfy`, not `Large`, failing-first); its production figure rises by the four-line guard | 1 inspection declaration retired (`workspace_sidebar_transition_pending_for_test`) into the evidence surface; 0 actuation seams added | **done**: `AdaptiveShellInputs` — constructed once per reconciliation and validated as a unit by the policy module | `ShellGeometryEvidence` via `shell_geometry_evidence()`. Its `persisted_workspace_fraction` / `persisted_properties_fraction` fields are the point: they make the allocation-path persistence rule **assertable** for the first time, where it was previously checkable only by reading the code or watching an animation stutter in the installed Flatpak | tier-3 — visual-sensitive, two named pixel invariants and the workspace-sidebar animation matrix | 7b | migrated |
