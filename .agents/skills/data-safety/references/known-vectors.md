...
**Current guardrail**: `filesystem::write::atomic_replace` probes metadata before temp creation, creates overwrite temps with no wider standard permissions than the destination, applies required metadata before the final temp-file sync, and syncs the parent directory after successful rename. `filesystem::write::move_durable()` (the cross-filesystem fallback for a rename) uses source metadata and removes the source only after the destination write and destination parent sync complete; `filesystem::write::copy_durable()` never removes its source. The retired `copy_file_durable` was a move under a copy's name — a primitive whose name says "copy" must never remove its source.
