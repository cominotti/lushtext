...
- Allocation handlers are on the animation frame path. They may enforce cheap runtime clamps, but they should not write GSettings, rebuild adaptive surface state, or repeatedly parse and reinstall `AdwBreakpoint` conditions. In LushText, that pattern made installed-Flatpak sidebar and file-info animations look low-refresh; caching the last allocated width and derived breakpoint threshold fixed the visual jank without changing the user-facing animation.
- **Deep Tree Layout Constraints**: When placing a `GtkListView` inside a `GtkScrolledWindow` inside a fixed-width paned, `propagate-natural-width` MUST be `false`. Deep tree indentation will otherwise increase the row's natural width beyond the paned boundary, expanding the sidebar uncontrollably. Additionally, text labels in the tree must use `EllipsizeMode::End` (or similar) so their minimum width yields to the container constraint rather than demanding their full text width.
