...
| GTK axioms | Isolated probes of the GTK/Adwaita behaviours geometry designs rely on (ledger A1–A20), plus one interactive/`--check` sample per probe | `crates/gtk-lush/axioms/` (probes, samples) and `crates/gtk-lush-adoption-lab/tests/axiom_probes.rs` (runner) | Private headless Mutter only | `make gtk-axioms` |
