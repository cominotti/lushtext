...
| GTK axioms | Isolated probes of the GTK/Adwaita behaviours geometry and window-lifecycle designs rely on (ledger A1–A21), plus one interactive/`--check` sample per probe | `crates/gtk-lush/axioms/` (probes, samples) and `crates/gtk-lush-adoption-lab/tests/axiom_probes.rs` (runner) | Private headless Mutter only | `make gtk-axioms` |
...
- CI runs the widget suite in shards from `scripts/widget-shards.py` (`Widget Tests (<shard>)` jobs). A new test in an existing module needs no table edit; a **new widget module file must be added to a shard** in that table, or `make check-widget-shards` fails. Reproduce one CI shard locally with `make test-widget-shard WIDGET_SHARD=<shard>`; `make test-widget` still runs everything.
