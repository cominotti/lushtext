...
- Every async result that changes UI state needs a freshness token plus an
  editor/window lifetime check. Prefer `gtk_lush_settle::Debounce` when it
  already owns the UI timer scheduling; use raw generation counters only for
  worker-result freshness that is not a UI timer. For path-sensitive work, also
  verify the editor still owns the same path and is still mounted in the tab
  view.
...
- A frozen snapshot used as a temporary paned child must preserve the live child's minimum width contract. If the snapshot host reports a smaller minimum width than the real sidebar, GTK may later warn while measuring the end `GtkBox` even though the root cause is the start-child snapshot surface.
- If the actual `GtkPaned` child is a stable host such as `GtkStack`, preserve the same width floor on that host too. Fixing only the descendant pane can still leave a one-pixel `GtkStack ... needs at least ...` warning behind.
- Stable hosts used only to swap live vs frozen children, such as `GtkStack`, must have their own transitions disabled unless a second animation is intentional. Otherwise they can quietly add hidden-child allocation work and reintroduce geometry warnings or stutter.
- Snapshot generation itself can be the responsiveness problem. If the frozen image is captured synchronously on the click path, hide-time animation can still stutter even after the geometry warning is fixed. Prefer refreshing or prewarming the snapshot during idle or another steady-state moment, then reuse that cached image during the actual interaction.
- A non-null cached paintable is not enough. If the frozen pane briefly turns black or empty, the snapshot may be visually invalid even though the paintable exists; validate the actual rendered image, not only pointer presence.
- For black or empty frozen panes, a persistent `GtkWidgetPaintable` whose `current_image()` stays warm can be safer than a one-shot snapshot captured at hide start. Treat "capture cost" and "snapshot validity" as separate concerns.
- Freeze only the pane that is actually expensive. If freezing the content pane introduces stretching or end-of-hide artifacts while the live content animates smoothly, keep the content live and freeze only the heavy sidebar subtree.
- If Adwaita split-view, sidebar, bottom-sheet, or file-info animations look like they are running below the monitor refresh rate in the installed Flatpak, inspect allocation and notify hot paths before assuming GPU/compositor trouble. In LushText, per-frame `size_allocate()` split-view synchronization, GSettings writes from `notify::sidebar-width-fraction`, and repeated `AdwBreakpoint` condition parsing/reinstallation caused visible low-refresh animation without any explicit blocking I/O. The fix is cheap comparisons, cached allocation widths/breakpoint thresholds, runtime-only geometry clamps, and persistence only after explicit user action or animation completion.
...
- Settle helper pattern: for UI debounce, superseding one-shots, and readiness-linked settle bursts, prefer `gtk_lush_settle::{Debounce, SupersedingTimer, SettleBurst}` over hand-rolled `Cell<u32>` timer gates. Worker-result freshness may still use explicit generation counters when no UI timer owns the token.
...
- [CONSIDER] Helper vs SourceId/raw counter — `gtk_lush_settle` avoids SourceId lifecycle bugs and keeps timer semantics consistent; prefer it for fitting new UI timer code
